...
<tr>
<td>Ẩn với viện phí (Ngoại trú)</td>
<td><input type="checkbox" id='HideVienPhiNgoaiTru'  value="1" name="HideVienPhiNgoaiTru"></td>
<td><!--<input type=checkbox id="angpp" value="1" name="IsHide4GPP">--></td> 
  <td>Hiển thị lô hạn dùng</td>
<td><input type="checkbox" id='ShowLoHanDung'  value="1" name="ShowLoHanDung"></td>
</tr>
<tr><td>SO_DANG_KY</td>
...
<td></td>
<td>MA_HOAT_CHAT(Theo thông tư)</td>
...
<td></td>
<tr><td>Thong tin thau BHYT</td>
<td><input type="text" id='ThongTinThauBHYT'   name="ThongTinThauBHYT" style="width: 290px!important"></td> 
<td></td>
<td>Giá BHYT Số lẻ <br/>(đối với trường hợp số lẽ <br/>nhập thêm giá vào đây)</td>
<td><input type="text" id='GiaBHYTSoLe'   name="GiaBHYTSoLe" style="width: 290px!important"></td>                   
</tr>               
</tr>
</table>
</div>
...
<input type="button" id="btn_excelbnsudungthuocnoitru" value="Excel DS BN sử dụng thuốc nội trú">
<input type="button" id="isThuocBHYT_isKhongSuDung" value="Hội chẩn">
</div>

<div id="dialog-xuatbnsudungthuocnoitru" title="Excel BN sử dụng thuốc nội trú" style="display:none;">
  Thuốc: <label class="excel_view_tenthuoc" style="font-weight: bold;"></label><br>
...
const url = 'resource.php?module=<?=$modules?>&view=<?=$view?>&hienmaloi=1';
const urlGetDefaultValue = url+'&action=data_get_hoichan';
const urlUpDateController = url+'&action=controller&oper=update_hoichan';
  const columnDialog={
  column:1,
  ...
  cancelText: 'Thoát', 
  Dialog:{  
  title: 'Cho sử dụng thuốc BHYT và Không cho kê đơn',
  width: 500,
  },
  Array:[
      // {type:'combobox',name:'ID_thuoc',validate:'require',defaultValue:'',label:'Tên Thuốc',comboboxoption:comboboxoption()},
      {type:'disabled',name:'TenGoc',defaultValue:false,label:'Tên gốc',isenter:false},
      {type:'checkbox',name:'isSuDungBHYT',defaultValue:false,label:'Sử dụng BHYT',isenter:false},
      {type:'checkbox',name:'isKhongDuocKeDon',defaultValue:false,label:'Không cho kê đơn hội chẩn',isenter:false},
      {type:'hidden',name:'ID_Thuoc',defaultValue:''}, 
    ]
  } 
jQuery(document).ready(function() {
  window.id_thuocxuatexcel=0;
$( "#dialog-xuatbnsudungthuocnoitru" ).dialog({
      resizable: false,
      autoOpen: false,
      width:400,
      modal: true,
      buttons: {
        "Xuất excel": function() {
           window.open("resource.php?module=report&view=<?=$modules?>&action=excel_bn_noitru_sudungthuoc&type=excel&tungay="+$("#tungay").val()+"&denngay="+$("#denngay").val()+"&id_thuoc="+ window.id_thuocxuatexcel);
        },
        "Đóng": function() {
          $( this ).dialog( "close" );
        }
...
$("#tungay").datepicker({
    ...
    onClose: function(selectedDate) {
      $('#tungay').attr('value', $("#tungay").datepicker({ dateFormat: 'dd-mm-yyyy' }).val());
    },
  ...
  $("#denngay").datepicker({
      ...
      dateFormat: 'dd/mm/y',    
      onClose: function(selectedDate) {
        $('#denngay').attr('value', $("#denngay").datepicker({ dateFormat: 'dd-mm-yyyy' }).val());
      },
      onSelect: function(dat, inst) {
      }
  });

  $("#btn_excelbnsudungthuocnoitru").click(function(){
    if(window.id_thuocxuatexcel==0){
      alert("Vui lòng chọn thuốc cần xuất excel.");
    }else{
      $( "#dialog-xuatbnsudungthuocnoitru" ).dialog('open');
    }
    
  });
  $("#isThuocBHYT_isKhongSuDung").click(function(){
    if(window.id_thuocxuatexcel==0){
      alert("Vui lòng chọn thuốc cần chỉnh sửa.");
    }else{
      // dialog hiện thị check info thông tin của thuốc
      fetchData(
      {
        'idThuoc':window.id_thuocxuatexcel,
      }, urlGetDefaultValue).then(function(data) {
        data = $.parseJSON(data);    
        data[0].TenGoc=window.tengocthuoc;
        DialogXuatExcel.edit(data[0]);
      });
// DialogXuatExcel.open();
// DialogThuoc.edit(Object.assign(gridThuoc.getSelectedData(), {   
//   oper:'edit',        
// }));
 ...
 $("#sua").button();
 $("#xoa").button();   
 $("#excel,#sua_giathuc").button();	 
  $("#btn_excelbnsudungthuocnoitru").button();   
  $("#isThuocBHYT_isKhongSuDung").button();   
 window.dataView;
 window.grid;
 window.data = [];
...
{name:'Gía lẻ BHYT',id:'GiaBHYTSoLe',field: "GiaBHYTSoLe",width: 90},
{name:'Ẩn với viện phí',id:'HideVienPhiNgoaiTru',field: "HideVienPhiNgoaiTru",width: 60,formatter: CheckboxFormatter}, //formatter: CheckboxFormatter
];
...
$(function () {	
 ...
 grid.onClick.subscribe(function (e, args) {
   window.id_thuocxuatexcel=dataView.getItem(args.row).id;
   // console.log(window.id_edit);
   window.tengocthuoc = dataView.getItem(args.row).TenGoc
    $(".excel_view_tenthuoc").html(dataView.getItem(args.row).TenGoc);
 ...
 grid.onDblClick.subscribe(function (e, args){
  ...
  if(dataView.getItem(args.row).isleaf==1 && permission['sua']==1){
    ...
    var ShowLoHanDung = dataView.getItem(args.row).ShowLoHanDung;
    ...
    var ThongTinThauBHYT = dataView.getItem(args.row).ThongTinThauBHYT; 
    var GiaBHYTSoLe = dataView.getItem(args.row).GiaBHYTSoLe; 
  	var HideVienPhiNgoaiTru = dataView.getItem(args.row).HideVienPhiNgoaiTru;	
	
    $("#GiaBHYTSoLe").val(GiaBHYTSoLe);
    $("#mathuoc").val(mathuoc);
    $("#MaBHYT").val(MaBHYT);
	...
	$("#ThongTinThauBHYT").val(ThongTinThauBHYT);
  ...
  if(ShowLoHanDung=="1"){
    $("#ShowLoHanDung").prop('checked',true);
  }else{
    $("#ShowLoHanDung").prop('checked',false);
  }
  if(HideVienPhi=="1"){
  ...
  if(HideVienPhiNgoaiTru=="1"){
    $("#HideVienPhiNgoaiTru").prop('checked',true);
  }else{
    $("#HideVienPhiNgoaiTru").prop('checked',false);
  }
  if(active=="1"){
...
$("#excel").click(function(){
 window.open("resource.php?module=report&view=<?=$modules?>&action=excel_dmThuoc&type=excel");
})
...
function check_n(){
  ...
  check_null["hesodieuchinhgiaban"]=dataToSend["hesodieuchinhgiaban"];          
  check_null["dongia"]=dataToSend["dongia"];  
  for(var key in check_null)
  { 
    if(check_null[key]==""){
		 window.kiemtra=false;
		 $("input[name="+key+"]").css("background-color","#F4FA58");
     if(key1==''){
...
{
    if (value==1)
        return '<input type="checkbox" name="" disabled="disabled" value="'+ value +'" checked />';
    else
        return '<input type="checkbox" name="" disabled="disabled" value="' + value + '" />';
}
</script>