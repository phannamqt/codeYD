...
function create_grid(){	
	 $("#rowed3").jqGrid({
		url:'pages.php?module=<?=$modules?>&view=<?=$view?>&action=data_tinhthanhpho',
		datatype: "json",	
		...
		colModel:[
			...
			{name:'TenTinhThanhPho',index:'TenTinhThanhPho', width:"15%", editable:true,align:'left',hidden:false,editrules: {required:true},edittype:"text",formoptions:{ rowpos:1, colpos:1}}, 
			   ...
			   afterSubmit : function(response, postdata) { 
				...
				afterShowForm : function (formid) {
					...
					$("#add_new").click(function(e){
					   $.post('pages.php?module=web_services&function=get_link&action=index&folder=danh_muc_quoctich:').done(function(data) {
			 	...
			 	afterShowForm : function (formid) {					
					...
					lendong(formid);			 
					 $("#add_new").click(function(e){
					  ...
					  links='pages.php?module=danhmuc&view=danh_muc_quoctich&id_form=47&id_loai=undefined';
					  elem=1 + Math.floor(Math.random() * 1000000000); 
					  width=90;
					  height=90;
...
function load_select1(){
	window.nation1 = $.ajax({url: 'pages.php?module=web_services&function=get_auto_edit_option&action=index&term=&status=2&tables=DM_QuocTich&id=ID_QuocTich&name=TenQuocTich', async: false, success: function(data, result) {if (!result) alert('Không load được danh mục quốc tịch');}}).responseText;	
...
function load_select(){
	window.nation = $.ajax({url: 'pages.php?module=web_services&function=get_auto_edit_option&action=index&term=&status=2&tables=DM_QuocTich&id=ID_QuocTich&name=TenQuocTich', async: false, success: function(data, result) {if (!result) alert('Không load được danh mục quốc tịch');}}).responseText;	
}
