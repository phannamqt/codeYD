...
function create_grid(){	
	 $("#rowed3").jqGrid({
		url:'pages.php?module=<?=$modules?>&view=<?=$view?>&action=data_kho',
		datatype: "json",	
		...
		{recreateForm:true,height:'auto',width:'auto',reloadAfterSubmit:false,url:'pages.php?module=<?=$modules?>&view=<?=$view?>&action=controller&oper=edit',closeOnEscape : true,modal: true,recreateForm: true,
			   afterSubmit : function(response, postdata) { 
				...
				afterShowForm : function (formid) {
					...
					//  links='pages.php?module=danhmuc&view=danh_muc_khohang&id_form=47&id_loai=undefined';
...
function load_select1(){
	window.phongchuyenmon = $.ajax({url: 'pages.php?module=web_services&function=get_auto_edit_option&action=index&term=&status=2&tables=DM_PhongChuyenMon&id=ID_PhongChuyenMon&name=TenPhongChuyenMon', async: false, success: function(data, result) {if (!result) alert('Không load được danh mục phòng ban');}}).responseText;	
