...
<script type="text/javascript">
    ...
    var dateTo=curr_date2 + "/" + curr_month2 + "/" + curr_year2;
        ...
        function create_grid_sucobynhanvien() {
            ...
            $("#rowed3").jqGrid({
                url: 'pages.php?module=<?= $modules ?>&view=<?= $view ?>&action=data_duyetlai&fromdate=<?= $_GET["fromdate"] ?>&todate=<?= $_GET["todate"] ?>',
                datatype: "json",
                colNames: ['IndexID', 'Ngày ', 'Bộ phận', 'Tên', 'Chi tiết công','Log Ko hợp lệ','Log trễ sớm',
                     'Đi trễ', 'Ra sớm', 'TongCong'],
                colModel: [
                ...
                caption: "Công chi tiết từ "+dateFrom +" đến "+ dateTo+"------ <label for='checkbox'style='width:110px' >Thu gọn</label><input type='checkbox' id='checkp'>",
