    ...
    <div class="thongtin_luotkham" id="right_col">
    	...
    	<div style="margin-top: 0px; margin-left:5px;" >
            ...
            <span style="margin-left:165px">
                  <label>Kiểu in: </label>
                  <select id="chonanh">
                  <option value="inngang" selected>In ngang</option>
                  <option value="indoc" >In dọc</option>
...
$(document).ready(function() {
	...
	$("#xemin").click(function(e){	
		...
		if($( "#chonanh option:selected" ).text()=="In dọc")
		{
			...
			{		
				dialog_pic_select("Chọn ảnh",90,"170px","jdshju8789789",window.search_string,"EEG",print_action,"PHIẾU ĐIỆN NÃO ĐỒ ĐIỆN TOÁN","ENCEPHALO - ELECTROGRAPHY","Bác sĩ CHẨN ĐOÁN","left",3,"EEG",'<?=$modules?>',"780");
			}else{
				dialog_pic_select("Chọn ảnh",90,"170px","jdshju8789789",window.search_string,"EEG",print_action,"PHIẾU ĐIỆN NÃO ĐỒ ĐIỆN TOÁN","ENCEPHALO - ELECTROGRAPHY","Bác sĩ CHẨN ĐOÁN","left",3,"EEG_khongchuky",'<?=$modules?>',"780");
				}
		...
		$(".frame_u78787878975f").css("width","793px");		
			...
			print_action="xemin";
			dialog_report("Xem trước khi in",90,90,"u78787878975f","pages.php?module=report&view=<?=$modules?>&action=EEG_InNgang&header=top&id_benhnhan="+id_benhnhan+"&id_kham="+_kham+"&type=report&id_form=10&tieude=PHIẾU ĐIỆN NÃO ĐỒ ĐIỆN TOÁN&chucnang=ENCEPHALO - ELECTROGRAPHY&chucdanh=Bác sĩ CHẨN ĐOÁN&chucdanh2=KỸ THUẬT VIÊN",'InKhoA4MauInNgang');	
		}
	});
	...
	resize_control();
	if(id_kham2!="0"){
		$.getJSON( "pages.php?module=<?=$modules?>&view=<?=$view?>&action=data_diennao2&idkham="+id_kham2+"&idbenhnhan="+id_benhnhan, 
			function( data ) {
				data_luotkham=data;
	...
	$.getJSON( "pages.php?module=<?=$modules?>&view=<?=$view?>&action=data_diennao&idbenhnhan="+id_benhnhan, 
		function( data ) {
			data_luotkham=data;
