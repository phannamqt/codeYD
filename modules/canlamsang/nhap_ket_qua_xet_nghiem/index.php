...
<div id="dialog-SID" title="In theo từng Sample ID (lần chỉ định)" style="display:none;">
 ...
 <div id="panel_main">    
        ...
        <div class="ui-layout-center top_form ui-widget-content" >
        	<div class="top">
				...
				<button style="float:right; margin-top: -30px; margin-right: 144px;"id="xeminSID">In SID</button>
            </div>
        		 ...
        		 setTimeout(function(){							
						   $.post('pages.php?module=<?=$modules?>&view=<?=$view?>&action=controller&oper=updatecannamnu&hienmaloi=3',dataToSend)
			                        .done(function( gridData ) {	
			                                            ...
			                                            $("#rowed6").jqGrid().setGridParam({datatype:'json',url:"pages.php?module=<?=$modules?>&view=<?=$view?>&action=data_danhsachketquaxetnghiem&idbenhnhan="+id_benhnhan+"&idluotkham="+id_luotkham}).trigger("reloadGrid");	
														 })
			                                            .fail(function() {
...
/*		 var tam3=$("#rowed6").jqGrid ('getCell', tam, 'CoLuuKQInRieng');
				  	var tam4=$("#rowed6").jqGrid ('getCell', tam, 'ID_LoaiKham');
					if(tam3!='0'){
						alert('Trong lượt này '+tam4+ 'có các XN có hình phải in riêng');
					}
				 */
		...
		$( "[id^=rowed6ghead_1_]" ).each(function(){
					...
					if($('#'+this.id +' td div').attr("cokqinrieng")!="0")
					{
						...
						tam= $('#'+this.id +' td div').attr("tenLK");
						alert(tam+' nhấn nút in riêng');
					}
       		});	
		...
		$.cookie("in_status", "print_preview"); 
		//console.log("NAM: "+$.cookie("os_name"));
		...
		mapForm.action = 'pages.php?module=report&view=<?=$modules?>&action=xetnghiem_ubuntu&header=top&id_benhnhan='+id_benhnhan+'&id_luotkham='+id_luotkham+'&type=report&id_form=10&inrutgon='+rutgon+'&chuky='+chuky+"&report_name=XetNghiem";
	...
	$("#xeminSID").click(function(e){	
		$.post('pages.php?module=<?=$modules?>&view=<?=$view?>&action=data_SID&id_luotkham='+id_luotkham).done(function(data){
			datax=data.split('|||');
			$("#nhomdv_body").empty();
			for(var i=1;i<datax.length;i++){
				//console.log(datax[i]);
				tam=datax[i].split(';');
				$("#nhomdv_body").append('<tr><td><input type="checkbox" id="ds_'+tam[0]+'" name="ds[]" class="ds-check" value="'+tam[0]+'" /></td><td>Sample ID '+'<span style="color:red">'+tam[0]+'</span> chỉ định lúc '+'<span style="color:blue">'+tam[1]+'</span> bởi '+tam[2]+'</td></tr>');
			}
			
		$("#dialog-SID").dialog("open");
		});
...
$( "#dialog-SID").dialog({
      ...
      buttons: {
        "IN": function() {
				...
				var d=0;
				var ids='';
				$('.ds-check:checked').each(function () {
					if(ids==''){
						ids=$(this).val();
					}else{
						ids+=";"+$(this).val();
				...
				if(d==0){
					tooltip_message("Vui lòng chọn SAMPLE ID cần in");
					window.damgluu=0;
				}else{
					//alert(ids);
		...
		$.cookie("in_status", "print_preview"); 
			dialog_report("Xem trước khi in",90,90,"u78787878975f","pages.php?module=report&view=<?=$modules?>&action=xetnghiem2&header=top&id_benhnhan="+id_benhnhan+"&id_luotkham="+id_luotkham+"&type=report&id_form=10&inrutgon="+rutgon+'&chuky='+chuky+"&sid="+ids,'XetNghiem');
			$(".frame_u78787878975f").css("width","793px");

			$("#dialog-SID").dialog("close");


					/*var r = confirm("Việc sao chép này sẽ làm thay đổi các hạng mục chưa được cấu hình riêng trong form 3 của các nhóm được sao chép này.\n Bạn có muốn tiếp tục thực hiện?");
					if (r == true) {
						$.post('pages.php?module=<?=$modules?>&view=<?=$view?>&action=controller&oper=dannhomdv&hienmaloi=a&ids='+ids+"&nhomnv="+window.idnhomnv+"&tunhom="+window.idnhomlsp).done(function(){
							window.damgluu=0;
							$("#dialog-SID").dialog("close");
							jQuery("#rowed5").jqGrid('setGridParam',{url:'pages.php?module=<?=$modules?>&view=<?=$view?>&action=data_loaikhamnhomLSP&id='+window.idnhomnv,datatype:'json'}).trigger("reloadGrid");
							tooltip_message("Đã lưu");
						});
					}else{
						window.damgluu=0;
	...
	$.get( "pages.php?module=web_services&function=create_panel1&id_benhnhan="+id_benhnhan+"&action=index", function( data ) {
	 ...
	 age= $('.profile_container:nth-child(3)  .texts').text();
	  $("#cannang").val($('.profile_container:nth-child(9)  .texts').text());  
	...
	$.getJSON( "pages.php?module=<?=$modules?>&view=<?=$view?>&action=data_all&idbenhnhan="+id_benhnhan, 
		function( data ) {
			data_luotkham=data;
...
function create_grid(){	
	...
	//var filekqir3 = "<input type='button' value='Lưu' onclick='filekqir3()'\>";
	 $("#rowed3").jqGrid({
		url:"pages.php?module=<?=$modules?>&view=<?=$view?>&action=data_loaixetnghiem&idbenhnhan="+id_benhnhan,
		datatype: "local",	
		colNames: ['','Tên xét nghiệm', 'Ngày chỉ định', 'Ngày hẹn trả','Nơi thực hiện','Vị trí lấy mẫu','Lưu'],
            colModel: [
                ...
                {name: 'NgayGioHenTraKQ',editable: false, index: 'NgayGioHenTraKQ',width:"15%"},
                {name: 'NguoiThucHien2',editable: false, width:"15%",index: 'NguoiThucHien2',edittype:"select", editoptions:{value:nguoichidinh},formatter: "select"},
                {name: 'ThongSoKyThuat',editable: false, index: 'ThongSoKyThuat',width:"20%"},
                
                {name: 'duongdanfile', index: 'duongdanfile', align: 'center',width:"7%", edittype: "button",hidden: true,formatter: function (cellValue, options, rowObject) {return "<input type='button' value='Lưu' onclick=\"filekqir3('" + rowObject[0] + "','" + rowObject[3]  + "','" + rowObject[4]  +"');\" />"}},
...
function create_grid2(){	
	 $("#rowed4").jqGrid({
		url:"pages.php?module=<?=$modules?>&view=<?=$view?>&action=data_ketqua&idbenhnhan="+id_benhnhan,
		datatype: "local",	
		colNames: ["id_kham","id_benhnhan","id_xetnghiem",'Loại xét nghiệm', 'Chỉ số', 'Kết quả','KQ LAB','Giá trị bình thường',''],
            colModel: [
                ...
                {name: 'KQHisLis', index: 'KQHisLis',align:'center', hidden: true,editable: false,width:50},
...
/*			$( "[id^=rowed4ghead_0_]" ).each(function(){
			...
			if($('#'+this.id +' td div').attr("id")=="3748_ketluan" || $('#'+this.id +' td div').attr("id")=="3816_ketluan" ||$('#'+this.id +' td div').attr("id")=="3845_ketluan" ||$('#'+this.id +' td div').attr("id")=="3881_ketluan" ||
				...
				$('#'+this.id +' td div').attr("id")=="3746_ketluan2" ||$('#'+this.id +' td div').attr("id")=="3747_ketluan2"|| $('#'+this.id +' td div').attr("id")=="6335_ketluan"|| $('#'+this.id +' td div').attr("id")=="6386_ketluan"
				|| $('#'+this.id +' td div').attr("id")=="6989_ketluan"
				){
...
caption:"<div style='float:right'><label class='diengiai'>Nhập KQ</label></div>"
	});
...
function create_grid3(){	
	var filekqir = '<button id="kqir" class="chonfile">File KQIR</button>';
	 ...
	 $("#rowed5").jqGrid({
		url:"pages.php?module=<?=$modules?>&view=<?=$view?>&action=data_ketquainrieng&idbenhnhan="+id_benhnhan,
		datatype: "local",	
...
function ketqua_xetnghiem() {
		  $("#rowed6").jqGrid({
            url:"pages.php?module=<?=$modules?>&view=<?=$view?>&action=data_danhsachketquaxetnghiem&idbenhnhan="+id_benhnhan,
            datatype: "local",
            colNames: ['','','','Ngày chỉ định','Loại XN','Chỉ số', 'Kết quả','Kết quả LAB' ,'Giá Trị BT', 'Người nhập', 'Người duyệt', 'Giờ KT','','In','','Hẹn','Nơi','','',''],
            colModel: [
                ...
                {name: 'KetQua', index: 'KetQua', width: "100%", align: 'center',  hidden: false,editable: false, formoptions: {}},
                {name: 'KQHisLis', index: 'KQHisLis', width: "100%", align: 'center',  hidden: true,editable: false, formoptions: {}},
				...
				{name: 'TenLoaiKham', index: 'TenLoaiKham', hidden: true},
           ...
           grouping: true,
            groupingView: {groupField: ['NgayGioTao', 'ID_LoaiKham'],
                ...
                groupColumnShow: [false, false],
               	...
               	<div cokqinrieng="{CoLuuKQInRieng}"  tenLK="{0}"    id="{ID_Kham}_{ID_LoaiKham};{1}" class="ui-state-error ui-state-hover subTotal2 "onClick=\"inKQNew({ID_LoaiKham},{1},{ID_Kham});\" style="border:0;display:none"><i class="fa fa-print css-print css-icon" aria-hidden="true" title="In KQ in riêng"></i></span></div>\
 ...
 $.post('pages.php?module=<?= $modules ?>&view=<?= $view ?>&action=controller&oper=xoaketqua&id_kham=' + id_kham).done(function(data)
        {
            if ($.trim(data) == '') {
            	...
            	$("#rowed3").jqGrid().setGridParam({datatype:'json',url:"pages.php?module=<?=$modules?>&view=<?=$view?>&action=data_loaixetnghiem&idluotkham="+id_luotkham+"&idbenhnhan="+<?=$_SESSION["ThongTinBenhNhan"]["id_benhnhan"]?>}).trigger("reloadGrid");	*/   
            			$("#rowed6").jqGrid().setGridParam({datatype:'json',url:"pages.php?module=<?=$modules?>&view=<?=$view?>&action=data_danhsachketquaxetnghiem&idbenhnhan="+id_benhnhan+"&idluotkham="+id_luotkham}).trigger("reloadGrid");	
						$("#rowed4").jqGrid().setGridParam({datatype:'json',url:"pages.php?module=<?=$modules?>&view=<?=$view?>&action=data_ketqua&idluotkham="+id_luotkham+"&idbenhnhan="+id_benhnhan}).trigger("reloadGrid");	
...
function inKQNew(id_loaikham,soluong,idkham){
	window.id_lk=parseInt(id_loaikham/soluong);
	window.id_k=parseInt(idkham/soluong);
	window.MaBenhNhan=0;
	console.log(id_lk);
	var allRowsInGrid = $('#rowed6').jqGrid('getRowData');
		 for (i = 0; i < allRowsInGrid.length; i++) {
		    ...
		    if(pid2==id_k){
                ...
                load_image($(this).attr("alt"));	
  			   ...
  			   _search_string=	window.MaBenhNhan+"_"+window.id_kham_rowed5+"_"+window.id_k;
...
function diasableALl()
{
		...
		//$("#dathuchien").button( {disabled:false});
					...
					//	$("#sangloctruocsinh").button( {disabled:false});
						...
						$("#renal_ldlc").button( {disabled:true});
        ...
        for (j = 0; j < ids.length; j++) {
           ...
           $.post('pages.php?module=<?=$modules?>&view=<?=$view?>&action=controller&oper=luu&hienmaloi=3',dataToSend)
			                        .done(function( gridData ) {	
			                                             ...
			                                             $("#rowed6").jqGrid().setGridParam({datatype:'json',url:"pages.php?module=<?=$modules?>&view=<?=$view?>&action=data_danhsachketquaxetnghiem&idbenhnhan="+id_benhnhan+"&idluotkham="+id_luotkham}).trigger("reloadGrid");	$("#rowed6").jqGrid().setGridParam({datatype:'json',url:"pages.php?module=<?=$modules?>&view=<?=$view?>&action=data_danhsachketquaxetnghiem&idbenhnhan="+id_benhnhan+"&idluotkham="+id_luotkham}).trigger("reloadGrid");	
						$("#rowed4").jqGrid().setGridParam({datatype:'json',url:"pages.php?module=<?=$modules?>&view=<?=$view?>&action=data_ketqua&idluotkham="+id_luotkham+"&idbenhnhan="+id_benhnhan}).trigger("reloadGrid");	
						$("#rowed3").jqGrid().setGridParam({datatype:'json',url:"pages.php?module=<?=$modules?>&view=<?=$view?>&action=data_loaixetnghiem&idluotkham="+id_luotkham+"&idbenhnhan="+id_benhnhan}).trigger("reloadGrid");	   
						$("#rowed5").jqGrid().setGridParam({datatype:'json',url:"pages.php?module=<?=$modules?>&view=<?=$view?>&action=data_ketquainrieng&idluotkham="+id_luotkham+"&idbenhnhan="+id_benhnhan}).trigger("reloadGrid");	
			                                            })
...
//	alert(P2);
	//alert(P3);
					...
					$.post('pages.php?module=<?= $modules ?>&view=<?= $view ?>&action=controller_ThucHien2&oper=update&hienmaloi=3',postData).done(function(data){
						...
						//$("#rowed3").jqGrid('setGridParam',{datatype: 'json',url:"pages.php?module=<?=$modules?>&view=<?=$view?>&action=data_khamby_idluotkham&id="+window.idluotkham}).trigger('reloadGrid');
						$("#rowed3").jqGrid().setGridParam({datatype:'json',url:"pages.php?module=<?=$modules?>&view=<?=$view?>&action=data_loaixetnghiem&idluotkham="+id_luotkham+"&idbenhnhan="+id_benhnhan}).trigger("reloadGrid");	   
...
$("#renal_ldlc").click(function(){
	...
	var weight_cal=$("#weight_1").val();
	 ...
	 for (i=0;i<=ids.length-1;i++){
				if($("#rowed4").jqGrid ('getCell', ids[i], 'ID_XetNghiem')=="22219"){//Triglyceride
					
					tg=$("#"+ids[i]+"_ketqua").val();
				}
				if($("#rowed4").jqGrid ('getCell', ids[i], 'ID_XetNghiem')=="22218"){//Cholesterol
	...
	if(Kq_createnin!="" && Kq_createnin!="n"){
		if($("#height_1").val()=="" || $("#height_1").val()=="0" || $("#weight_1").val()=="" || $("#weight_1").val()=="0"){
			alert("Muốn tính RENAL FUNCTION, yêu cầu nhập chiều cao và cân nặng để tính toán!");
		}
		else{
	 ...
	 chol2="";
	 hdlc2="";
	for (i=0;i<=ids2.length-1;i++){//lấy giá trị từ lưới và gán biến
