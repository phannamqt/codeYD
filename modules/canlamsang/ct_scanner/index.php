...
<body>
 <div class="top_form ui-widget-content" >
    ...
    <div class="thongtin_luotkham" id="right_col">  	 
    	...
    	<a href="#" id="ct_sanner_report_english" class="fm-button ui-widget ui-state-default ui-corner-all fm-button-icon-left" style="margin-left:0px;  margin-bottom:1px;float: right; ">English<span class="ui-icon ui-icon-note"></span></a>	
    	</div>
 ...
 <div id="panel_main">    
	...
	<!-- English  -->
     <div id="dialog_english" title="Support translate / Hỗ trợ dịch thuật" style="display:none">
        <fieldset style="width: 43%; float: left">
        <legend class="title_fd" style="color: red;">CT Scanner tiếng anh</legend>
            <table border="0" style="width: 100%; float: left;">
                <tr>
            		<td width="50%" class="thongtin_dl">
            			<label class="title_in">+ DESCRIPTIONS:</label>   
                        <textarea style="width: 96%; height: 390px;" id="mota_eng" name="mota_eng" lang='luu'></textarea> 
            		</td>
            		<td width="50%" class="thongtin_dl">
            			<label class="title_in">+ SPECIFICATIONS:</label>   
                        <textarea style="width: 96%; height: 105px;" id="thongsokt_eng" name="thongsokt_eng" lang='luu'></textarea>
                        <label class="title_in">+ DIAGNOSIS:</label>   
                        <textarea style="width: 96%; height: 105px;" id="ketluan_eng" name="ketluan_eng" lang='luu'></textarea>
                        <label class="title_in">+ MANAGEMENT:</label>   
                        <textarea style="width: 96%; height: 105px;" id="loikhuyen_eng" name="loikhuyen_eng" lang='luu'></textarea>
            		</td>
            	</tr>
            </table>
        </fieldset>
        <!-- các nút button -->
        <table border="0" style="width: 10%;float: left;"">
            <tr>
                <td>
                <center>
                    <a style="margin-top: 100px;" href="#"  id="chen_eng" class="button_mid ui-widget ui-state-default ui-corner-all"><center><span class="ui-icon ui-icon-copy"></span>Chèn</center></a>
                    <a href="#"  id="clear_eng" class="button_mid ui-widget ui-state-default ui-corner-all"><span class="ui-icon ui-icon-arrowrefresh-1-s"></span>Clear</a>
                    <a href="#"  id="save_eng" class="button_mid ui-widget ui-state-default ui-corner-all"><span class="ui-icon ui-icon-disk"></span>Save</a>
                    <a href="#"  id="print_eng" class="button_mid ui-widget ui-state-default ui-corner-all"><span class="ui-icon ui-icon-print"></span>Print</a>
                    <a href="#"  id="close_eng" class="button_mid ui-widget ui-state-default ui-corner-all"><span class="ui-icon  ui-icon-closethick"></span>Close</a>
        ...
        <!-- end các nút button -->
        <fieldset style="width: 43%;float: right">
        <legend class="title_fd" style="color: green;">CT Scanner tiếng việt</legend>
            <table border="0" style="width: 100%; float: right;">
            	<tr>
            		<td width="50%">
            			<label class="title_in2">+ Mô tả:</label>   
                        <textarea style="width: 96%; height: 390px;" id="mota_1" name="mota_1"></textarea> 
            		</td>
            		<td width="50%">
            			<label class="title_in2">+ Thông số kĩ thuật:</label>   
                        <textarea style="width: 96%; height: 105px;" id="thongsokt_1" name="ketluan_1"></textarea>
                        <label class="title_in2">+ Kết luận:</label>   
                        <textarea style="width: 96%; height: 105px;" id="ketluan_1" name="ketluan_1"></textarea>
                        <label class="title_in2">+ Lời khuyên:</label>   
                        <textarea style="width: 96%; height: 105px;" id="loikhuyen_1" name="ketluan_1"></textarea>
            		</td>
            	</tr>
            </table>
        </fieldset>
...
$("#xemin").click(function(e){	
    ...
    $("#ct_sanner_report_english").click(function(e){
        // alert("11");
        $("#dialog_english").dialog("open");
    });
    $("#close_eng").click(function(e){
        $("#dialog_english").dialog("close");
        // alert("report_english");
    });
    $("#chen_eng").click(function(){
        $("#mota_eng").val($("#mota_1").val());
        $("#thongsokt_eng").val($("#thongsokt_1").val());
        $("#ketluan_eng").val($("#ketluan_1").val());
        $("#loikhuyen_eng").val($("#loikhuyen_1").val());
    });
    $("#clear_eng").click(function(){
        $("#mota_eng").val("");
        $("#ketluan_eng").val("");
        $("#loikhuyen_eng").val("");
        $("#thongsokt_eng").val("");
    });
    $("#print_eng").click(function(){
        dialog_report("Xem trước khi in",90,90,"u78787878975f","pages.php?module=report&view=<?=$modules?>&action=ct_scanner_eng&header=top&id_benhnhan="+id_benhnhan+"&id_kham="+id_kham+"&type=report&id_form=10&chuky="+chuky,'ct_scanner_eng');
		$(".frame_u78787878975f").css("width","793px");
    }); 

    $('#save_eng').click(function (){
        phancach = "";
        i = 0;
        dataToSend = '{';
        $('.thongtin_dl').find(':input[type=text],select,textarea,input[type=hidden]').each(function() {
        ...
        $.post('pages.php?module=<?=$modules?>&view=<?=$view?>&action=controller&oper=luu_eng&hienmaloi=3',dataToSend)
		...
		$("#left_col div div").click(function(e) {
				...
				for(i=0;i<val.length;i++){
					...
					if(tam==tam1){	
                        ...
                        //English
                        $("#mota_1").val(val[i]["cell"][6]);
						$("#thongsokt_1").val(val[i]["cell"][18]);//thong so ky thuat
						$("#ketluan_1").val(val[i]["cell"][7]);
                        $("#loikhuyen_1").val(val[i]["cell"][8]); 
				...
				load_image($(this).attr("alt"));
				////////////select kham_english
                $.getJSON( "pages.php?module=<?=$modules?>&view=<?=$view?>&action=data_kham_english&id_kham="+id_kham,
                    function( data ) {
                        data_luotkham4=data;
                        var tam1_cls="";
                        var data_mota_eng="";
                        var data_ketluan_eng="";
                        var data_loikhuyen_eng="";
                        var data_thongsokt_eng="";
                        $.each( data, function( key, val ) {
                            for(i=0;i<val.length;i++){
                                tam1_cls=val[i]["cell"];
                                data_mota_eng=tam1_cls[0];
                                data_ketluan_eng=tam1_cls[1];
                                data_loikhuyen_eng=tam1_cls[2];
                                data_thongsokt_eng=tam1_cls[3];
                            }
                        });
                        $("#mota_eng").val(data_mota_eng);
                        $("#thongsokt_eng").val(data_thongsokt_eng);
                        $("#ketluan_eng").val(data_ketluan_eng);
                        $("#loikhuyen_eng").val(data_loikhuyen_eng);
