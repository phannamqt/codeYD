...
<div id="main_content">
	...
	<div style="display:none;"><input type="radio" id="thuochangmuckham" name="thuocnhom" value="1" style="margin-left:30px!important;"/><label>Thuộc PB chuyên môn</label>      </div>
...
function create_grid() {
	...
	$("#rowed3").jqGrid({
		datastr:{},
		datatype: "jsonstring",
		colNames: ['LuotKham','ID loại khám', 'Mã BN', 'Họ tên', 'Tuổi', 'Giới tính', 'Hạng mục khám', 'Giờ hẹn', 'Giờ chỉ định','Chỉ định','Ghi chú','Gọi khám','NotesStatus','ID Bệnh nhân ','Tên Form','ID loại khám ','Gọi loa','MaYTe'],
		colModel: [
            ...
            loadComplete: function(data) {
				...
				for (i = 0; i < ids.length; i++) {
					...
					thoigianMax=thoigianMax.addMilliseconds(-500);
									...
									GoiLoa = "<button class='custom_button ' style='height:22px;' onclick=\"XepHangSendSocket('<?=$_SERVER['SERVER_NAME']?>','0000','" + rowData["MaYTe"] + "','" +rowData["ID_LuotKham"]+ "','0','');\" >Gọi loa</button>";
...
function create_grid2()
{
	$("#rowed4").jqGrid({
		datastr: {},
		datatype: "jsonstring",
		colNames: ['ID khám','ID Lượt khám', 'Mã BN', 'Họ tên', 'Tuổi', 'Giới', 'Hạng mục khám', 'Giờ c.định', 'Giờ hẹn KQ','Chỉ định','T.hiện','Ghi chú','Nhập KQ','Trạng thái','NotesStatus','ID Bệnh nhân','Tên form','Gọi loa','MaYTe'],
		colModel: [
