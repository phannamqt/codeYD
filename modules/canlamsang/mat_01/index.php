...
<body>
    ...
    <li id='list1'><a id="lydo_hoantat_tre" href="#"><span class="ui-icon  ui-icon-circle-plus"></span>Lý do hoàn tất trễ</a></li>
    <!-- <li id='list2'><a id="lydo_hoantat_tre1" href="#"><span class="ui-icon ui-icon-locked"></span>Huy đẹp trai</a></li> -->
  </ul>
...
<div class="thongtin_chidinh" style="width:687px!important">
    ...
    <tr>
      <td>
        <label style="width:80px;text-align:right">Người chỉ định:</label>
        <input lang='luu' name="nguoi_chidinh" style="width:80px" type="text" id="nguoi_chidinh" disabled>
      </td>
      <td>
      <label style="width:98px;margin-left:10px;">Người thực hiện:</label>
        <span class="custom-combobox">
        <input id="nhanvien" name="nhanvien"  type="text" style="width:80px;">
        </span>
        <input id="nhanvien1" name="nguoithuchien" type="text" lang='luu' style="display:none">
      </td>
      <td>
        <label style="width:98px; margin-left:10px;">Bs chẩn đoán:</label>
        <span class="custom-combobox">
    ...
    <tr>
      <td>        <label style="width:80px;text-align:right">Ngày chỉ định:</label>
        <input name="ngaychidinh" lang='luu' style="width:80px" type="text" id="ngaychidinh" disabled></td>
      <td>
        <input id="giothuchien" name="giothuchien" type="text" lang='luu' class="thongtin_thai zodi" style="width: 90px;margin-left: 10px; margin-top: 1px;">
      ...
      <td><input id="giohoantat" name="giohoantat" type="text" lang='luu' class="thongtin_thai zodi" style="width: 90px;margin-left: 10px;">
        <a href="#" id="hoantat" class=" ui-widget ui-state-default ui-corner-all fm-button-icon-left" style="margin-left:0px;  margin-bottom:1px; width: 80px; ">Hoàn tất<span class="ui-icon ui-icon-check"></span></a></td>
    </tr>
    </table>
    </div>
</div>
...
$(document).ready(function() {	
	...
	openform_shortcutkey();
		$('#giothuchien,#giohoantat').datetimeEntry({
...
$("#xemin").click(function(e){	
			if(_id_trangthai=="DangKham"|| _id_trangthai=="DangCho"){
					...
					$('.template_title_button').button( {disabled:true});
                         		...
                         		$( "#giothuchien" ).attr("disabled",true);
                         		  $( "#chuandoan" ).attr("disabled",true);
                         		  $( "#giohoantat" ).attr("disabled",true);
                 			...
                 			$('.template_title_button').button( {disabled:true});
						        ...
						        dataToSend += phancach + '"id_trangthai":"' + _id_trangthai + '"';
						         ...
						         dataToSend += phancach + '"id_luotkham":"' +_id_luotkham2+ '"';
						          dataToSend += phancach + '"giothuchien_hen":"' +$("#giothuchien").val()+ '"';
						        dataToSend += '}';
						        //alert(dataToSend);
						        dataToSend = jQuery.parseJSON(dataToSend);
						       //alertObject(dataToSend); 
                            ...
                            $('#mota').attr("disabled", "disabled");
                			 ...
                			 $('.template_title_button').button( {disabled:true});
						        ...
						        dataToSend += phancach + '"id_trangthai":"' + _id_trangthai + '"';
						         ...
						         dataToSend += phancach + '"id_luotkham":"' +_id_luotkham2+ '"';
						          dataToSend += phancach + '"giohoantat_hen":"' +$("#giohoantat").val()+ '"';
						        dataToSend += '}';
						        //alert(dataToSend);
						        dataToSend = jQuery.parseJSON(dataToSend);
						       alertObject(dataToSend); 
			...
			$("#add_template").click(function(e){
			  ...
			  $('#luu').click(function (){
				...
				$("#DM_template").click(function(e) {
					...
					// chuột phải hiện thị list
    $(document).bind("mouseup", function(e) {
    $('#menu2').hide();
    e.preventDefault();
    return false;
})
	$('#giohoantat').bind('contextmenu', function (e) {
    e.preventDefault();
                             ...
                             $('#loikhuyen').attr("disabled",false);
                              $('#xoamota').button( {disabled:false});
                               ...
                               $('.template_title_button').button( {disabled:false});
                         		...
                         		$('.nhanvien_button').button( {disabled:false});
                         		$( "#nhanvien" ).attr("disabled",false);
                         		$( "#giothuchien" ).attr("disabled",false);
								$( "#giohoantat" ).attr("disabled",false);
                         		  $( "#chuandoan" ).attr("disabled",false);
                         		  $( "#giohoantat" ).attr("disabled",false);
                         		  $('#add_template').button( {disabled:false});
...
/// huy thêm load thời gian vào để kiểm tra tính giờ hẹn
...
function kt_trangthai(id_thai){
	...
	var timebd = (date).split(' ').shift();
      	var datebd = (date).split(' ').pop();
      	datebd = datebd.split('/').reverse().join('/');
      	var datetime = datebd+' '+timebd;
