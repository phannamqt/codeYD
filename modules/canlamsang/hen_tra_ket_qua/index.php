...
<div id="dialog-form-2" style="display:none;">
...
function create_grid(){	
	 ...
	 $("#rowed3").jqGrid({
		  url: 'pages.php?module=<?= $modules ?>&view=<?= $view ?>&action=data_hentrakq&idluotkham=<?=$_GET["idluotkham"] ?>',
		datatype: "json",	
	...
	//
		...
		$.get( "pages.php?module=web_services&function=create_panel1&action=index&id_benhnhan="+idbenhnhan, function( data ) {
...
function   save2(){
		$("#save2").bind("click", function(e) { 
				...
				if($('#datepicker1').val()==''&&rowData['NgayGioTraKQ']!='')
					...
					{
						$.post('pages.php?module=<?= $modules ?>&view=<?= $view ?>&xoaID=0&action=data_trakq_update&ID_LuotKham='+rowData["ID_LuotKham"]+'&date_NgayGioTraKQ='+$("#datepicker1").val()+'&time_NgayGioTraKQ='+$("#timepicker1").val()).done(function(data){ 
							if ($.trim(data) == '') {
								tooltip_message("<?php get_text1("sua_thanhcong") ?>");
								$('#rowed3').setGridParam({loadonce: false, datatype: "json"}).trigger('reloadGrid');
							}
