...
$(document).ready(function() {	
	$("#open_form_hentra").click(function(e){
					  ...
					  var links='pages.php?module='+folder[0]+'&view='+folder[1]+'&id_form='+folder[2]+"&idluotkham="+id_luotkham;				  				  
	...
	$("#xemin").click(function(e){	
		dialog_report("Xem trước khi in",90,90,"u78787878975f","pages.php?module=report&view=<?=$modules?>&action=khamphukhoa&header=top&id_benhnhan="+id_benhnhan+"&id_kham="+id_kham+"&type=report&id_form=10&chuky="+chuky,'KhamPhuKhoa');
...
//para 
	...
	$.getJSON( "pages.php?module=<?=$modules?>&view=<?=$view?>&action=data_phukhoa&idbenhnhan="+<?=$_SESSION["ThongTinBenhNhan"]["id_benhnhan"]?>, 
		function( data ) {
			data_luotkham=data;
...
$("#add_templatekn,#add_templatepptt,#add_template,#add_templatektt,#add_templatecd,#add_templatedn").removeClass("ui-corner-all");
	...
	$("#laydulieu").click(function(){
		$.post('pages.php?module=<?=$modules?>&view=<?=$view?>&action=data_dl_canlamsang&ID_LuotKham='+id_luotkham).done(function(data){
...
function load_complete1(){
	...
	if(window.test2=="dathuchien1"){
			       $.post('pages.php?module=<?=$modules?>&view=<?=$view?>&action=controller&oper=luuthuchien&hienmaloi=3&id_benhnhan='+id_benhnhan,datalocalToSend)
			         ...
			         if(window.test2=="hoantat1"){
			         	$.post('pages.php?module=<?=$modules?>&view=<?=$view?>&action=controller&oper=luuhoantat&hienmaloi=3&id_benhnhan='+id_benhnhan,datalocalToSend)
								 .done(function( gridData ) {	
								 	delete window.datalocalToSend; 
								 							tooltip_message("Đã lưu");
			                                            testa();
			       ...
			       $.post('pages.php?module=<?=$modules?>&view=<?=$view?>&action=controller&oper=luudangkham&hienmaloi=3&id_benhnhan='+id_benhnhan,datalocalToSend)
								 .done(function( gridData ) {	
								 	delete window.datalocalToSend;
								 	tooltip_message("Đã lưu");
			                                            testa();	 
...
if(typeof window.datalocalToSend=='undefined'){
	...
	$.post('pages.php?module=<?=$modules?>&view=<?=$view?>&action=controller&oper=hoantat&id_benhnhan='+id_benhnhan,datalocalToSend)
	.done(function( gridData ) {	
		 //reloadform(); 
		 testa();
		 delete window.datalocalToSend;
...
$.post('pages.php?module=<?=$modules?>&view=<?=$view?>&action=controller&oper=dathuchien&id_benhnhan='+id_benhnhan,datalocalToSend)
.done(function( gridData ) {	
	//reloadform(); 
	testa();
	delete window.datalocalToSend;
