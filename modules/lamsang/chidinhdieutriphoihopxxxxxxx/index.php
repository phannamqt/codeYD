...
Vui lòng chọn một lượt khám để thực hiện chức năng này!!!
 ...
 <div  class="data_chongoikham" style="width:40%" title="Chọn gói khám" style="display:none">
 	<div class="left_col2" style="width: 38%;float: left;"><table id="data_goikham" ></table></div>
	<div class="center_col2" style="width: 59%;float: left;"> <table id="data_loaikham" style="float: left;" ></table> </div>
  ...
  <div  class="xemtoathuoc" title="Xem toa thuốc">
   ...
   <input type="hidden" id="idluotkham" value="<?php if(isset($_GET['idluotkham'])){ echo $_GET['idluotkham'];} ?>" /> 
	...
	<div class="ui-layout-west ui-widget-content left_col">   <table id="rowed3" ></table></div>
        ...
        <div id='n_menu'>
	        <div id='n_menu_left'>
		        <a  id="chitietquota" style="margin-left: 5px; padding-left: 110px;" href="#" > Chi tiết quota</a>
	        ...
	        <a  id="chongoikham" class="fm-button ui-state-default ui-corner-all " style="float: left; margin-left: 10px;" > Chọn gói khám</a>
   		</div>
    ...
    <div id='botton' >
    	<div id="bt_left" style=" min-width: 380px; ">
           ...
           <div id="bt_right">
				...
				<a  id="btn_inphieucd" class="fm-button ui-state-default ui-corner-all " style=" margin-left: 1%;  margin-top: -6px;"  > Xem in CĐ</a>
				<a  id="btn_incd" class="fm-button ui-state-default ui-corner-all " style=" margin-left: 1%;  margin-top: -6px;"  > In CĐ</a>
				...
				<?php
				if($_SESSION['user']['id_user']==50 || $_SESSION['user']['id_user']==178 || $_SESSION['user']['id_user']==785){
				 ?>
				<button id="btn_save_code" title="Lưu mã code giới thiệu">Code</button>
				<?php
				}
				?>
        	</div>
	...
	<div class="dialog_chongoikham" style="width:32%" title="<?php lang('chongkc')?>"  style="display:none">
		<!-- <div class="left_col2" style="width: 35%;float: left;"><table id="data_goikham" ></table></div>
    	<div class="center_col2" style="width: 59%;float: left;padding-left: 15px;">  <table id="data_loaikham" ></table> </div> -->
	</div>
...
var report_name=["Phiếu chỉ định điều trị"];
    jQuery(document).ready(function() {
	 ...
	 <?php 
		...
		echo 'window.n_hotrobhyt="'.$tam[0]['HoTroBHYT'].'";';
		...
		echo 'window.user_login='.$_SESSION['user']['id_user'].';';
	?>
		...
		$.get( "pages.php?module=web_services&function=create_panel1&action=index&id_benhnhan="+window.idbenhnhan, function( data ) {
		...
		$("#rowed4").jqGrid('setGridParam',{datatype: 'json',url:"pages.php?module=<?=$modules?>&view=<?=$view?>&action=data_hangmucloaikham_all&phantram="+window.phantram+'&trangthaikham='+window.n_trangthaikham+'&idluotkham='+window.idluotkham+'&nguoinuocngoai='+window.n_nguoinuocngoai+'&doituong='+window.loaidoituongkham}).trigger('reloadGrid');
		$("#xemtatca").change(function(event) {
			if($("#xemtatca").is(':checked')==true){
				...
				//console.log('N:'+id);
				$("#rowed4").jqGrid('setGridParam',{datatype: 'json',url:"pages.php?module=<?=$modules?>&view=<?=$view?>&action=data_hangmucloaikham_idnhomcls&id="+id+'&trangthaikham='+window.n_trangthaikham+'&idluotkham='+window.idluotkham+'&phantram='+window.phantram+'&nguoinuocngoai='+window.n_nguoinuocngoai+'&doituong='+window.loaidoituongkham}).trigger('reloadGrid');
				$("#gs_TenLoaiKham").val("");
				$('#rowed3 #'+id).focus();				 
		...
		$('#btn_inphieucd,#btn_incd, #btn_luu,#chongoikham,#hentrakq,#btn_chidinh').button();
		$('body').bind('keydown', function (e) {
			if(jwerty.is("ctrl+shift+f",e)){
				if(window.chophepluu==0 && (window.user_login==178 || window.user_login==785)){
					//console.log(1);
					$('#btn_luu').button('enable');
					window.chophepluu=1;
				}else if(window.chophepluu==1 && (window.user_login==178 || window.user_login==785)){
					//console.log(2);
		...
		$("#rowed5").jqGrid('setGridParam',{datatype: 'json',url:"pages.php?module=<?=$modules?>&view=<?=$view?>&action=data_khamby_idluotkham&id="+$("#idluotkham").val()}).trigger('reloadGrid');
	
		$("#hentrakq").click(function(e){
			links='pages.php?module=canlamsang&view=ds_xephang_canlamsang&action=ds_hentraKQ3&type=test&id_form=123&idluotkham='+$("#idluotkham").val();
...
$("#btn_incd").click(function(){
	window.inchidinh=1;
	//alert(window.inchidinh);
	window.xemin=0;
	$('#btn_incd').button('disable');
	$.post('pages.php?module=<?= $modules ?>&view=<?= $view ?>&action=check_thanhtoan&idluotkham=<?=$_GET['idluotkham']?>').done(function(data){
		var datacheck=$.trim(data);
		if(datacheck=='0' && window.n_khoachidinh!=1){
			$("#btn_luu").click();
	 ...
	 $("#rowed5 tbody").addClass('bongmo');
				...
				if($("#idluotkham").val()==""){
					...
					for(var i=0;i<=ids.length-1;i++){
						...
						if(rowData['TrangThai']!="HuyBo"){
							var _Code=$("#"+ids[i]+"_Code").val();
						}else{
							var _Code=JSON.stringify(rowData['Code']);
						}
						...
						dataToSend += '"Code":' + JSON.stringify(_Code);
						dataToSend += '}';
						phancach=',';
					//alert(dataToSend);
					}
				   ...
				   $.ajax({
						type: 'POST',
						async : false,
						url: 'pages.php?module=<?= $modules ?>&view=<?= $view ?>&action=controller_gioithieu&oper=add&hienmaloi=3',
						data: postData,
		...
		if((datacheck=='0' && window.n_khoachidinh!=1) || window.chophepluu==1){
				...
				if($("#idluotkham").val()==""){
				   ...
				   $.ajax({
						type: 'POST',
						async : false,
						url: 'pages.php?module=<?= $modules ?>&view=<?= $view ?>&action=controller&oper=add&hienmaloi=3',
						data: postData,
						success: function(data, status, xhr) {
							$("#rowed5 tbody").removeClass('bongmo');
							$("#n_dangluu" ).hide();
					 ...
					 /*$.post('pages.php?module=<?= $modules ?>&view=<?= $view ?>&action=controller&oper=add&hienmaloi=3',postData).done(function(data){
							$("#rowed5 tbody").removeClass('bongmo');
							$("#n_dangluu" ).hide();
							$("#btn_cddatra").prop( "checked", true );
							$("#rowed5").jqGrid('setGridParam',{datatype: 'json',url:"pages.php?module=<?=$modules?>&view=<?=$view?>&action=data_khamby_idluotkham&id="+$("#idluotkham").val()}).trigger('reloadGrid');
...
$('#dialog').dialog({
	...
	$('#dialog2').dialog({
		...
		buttons: {
			"Ok": function() {
				...
				$("#rowed5").jqGrid('setRowData',$("#idk2").val(),{HoTro:0});
				$("#rowed5").jqGrid('setRowData',$("#idk2").val(),{BNChiTra:0});
	...
	if($("#idluotkham").val()==""){
		...
		$(".data_chongoikham" ).dialog({
				...
				//if(window.n_dathanhtoanbill!=1 && window.n_khoachidinh!=1){
					...
					for(i=0;i<tmp.length;i++){
					   ...
					   if((window.loaidoituongkham=="BHYT") && (rowData["GiaTheoBaoHiem"]>0 )){
							...
							window.phantram_luu=window.phantrammoi[0];
							if(window.phantrammoi[1]>0){
								window.phantrammoi=	rowData["PhanTramCungChiTra"];
							}else{
								window.phantrammoi=window.phantrammoi[0];
							}
					  ...
					  var phithuchien=0;
						for(var i=0;i < tmp3.length;i++){ 
							...
							if(window.loaidoituongkham=="BHYT" && rowData['TrangThai']!='HuyBo'){
								if(rowData["Color"]=="1"){
						...
						tinhtongtien();
				}else{//end if trang thai
					alert_khoa(datacheck);
				}
				});//end post check thanhtoan
			  },
		  ...
		  open: function() {
		  	$("#data_goikham ").setGridWidth($(".left_col2").width());
			$("#data_goikham").setGridHeight($(".center_col").height()-60);
			$("#data_loaikham ").setGridWidth($(".center_col2").width());
			$("#data_loaikham").setGridHeight($(".center_col").height()-60);
		  },
		});
...
var lastsel;
    ...
    function create_grid2(){
        ...
        $("#rowed4").jqGrid({
            data: mydata,
            datatype: "local",
            colNames: ['GiaThueNgoaiThucHien', 'ThoiGianTrungBinhThucHien', 'ID_LoaiKham', 'Tên loại khám', 'Đ.Giá Viện phí', 'Đ.Giá Viện phí','Đ.Giá BHYT','P.Thu BV','P.Thu tại nhà','Hệ số người nước ngoài','BH trả','','ID_NhomCLS','','','Hỗ trợ','TenBHYT'],
            colModel: [
				...
				{name:'HoTro',index:'HoTro', width:"30%", editable:true,align:'right',edittype:"text",hidden:false,formatter:'currency',formatoptions:{decimalSeparator:".", thousandsSeparator: ",", decimalPlaces: 0, prefix: "", defaultValue: '0'}},
				{name: 'TenBHYT', index: 'TenBHYT', search: false, width: "20%", editable: false, align: 'left', hidden: true},   
				],
            loadonce: true,
            scroll: 1,
            modal: true,
			 ...
			 for(var i=0;i < tmp3.length;i++){ 
				var rowData = $("#rowed4").getRowData(tmp3[i]);	
			 	if(rowData["Color"]=="1"){
					...
					$("#rowed4").jqGrid('setRowData', tmp3[i], false, { background: '#DE5145',border:'1px solid #dfd9c3' });
				}
			 }
...
function create_grid3(){
        ...
        $("#rowed5").jqGrid({
			...
			editurl: 'clientArray',
            datatype: "local",
            colNames: ['BH','ID Loại Khám','Xóa','Loại khám', 'Đ.Giá V.Phí', 'Đ.Giá BHYT','Phụ thu','SL/ngày','Số ngày', 'BHYT trả','Hỗ trợ','BN trả', 'Phí T.Hiện', 'T.Thái', 'Thực hiện', 'Người CĐ', 'Người CĐ',  'GiaThueNgoaiThucHien',  'ThoiGianTrungBinhThucHien',  'IDKham',  'IDPhongChuyenMon',  'MaBenhNhan','LyDoHuyBo','huy',  'IDLuotKham','Luu','IDNhomCLS','NgayGioTao','TongTienTheoNhom' ,'ID Nhom PHY va Nhom DieuTriPhoihop','lần chỉ định','% cùng chi trả','TT cùng chi trả','','','','Code','','TenBHYT'],
            colModel: [
				{name:'FixGiaBHYT',index:'FixGiaBHYT', width:"6%", editable:true,stype: 'text',search:true,searchoptions: { sopt: ['eq', 'ne'],value:':;1:Có;0:Không'}, editable:true,align:'center',hidden:false,edittype:"checkbox",formatter:"checkbox",formatoptions:{disabled: false},formoptions:{ rowpos:13, colpos:1}
			,editoptions: { value:"1:0",dataEvents:  [{ type: 'click', fn: function(e) { 
				...
				if(IDKham==''){
				 //  alert('Hiện tại chức năng này chỉ thực hiện các chỉ định đã lưu');
					...
					if((datacheck=='0' || datacheck=='5') && window.n_dathanhtoanbill!=1){
						$.post('pages.php?module=<?= $modules ?>&view=<?= $view ?>&action=controller_fix_bhyt&hienmaloi=a&id='+n_idfix+'&nhom='+IDNhomCLS+'&ac='+tthai).done(function(data) {
							$("#rowed5").jqGrid('setGridParam',{datatype: 'json',url:"pages.php?module=<?=$modules?>&view=<?=$view?>&action=data_khamby_idluotkham&id="+$("#idluotkham").val()}).trigger('reloadGrid');
							$("#rowed5 tbody").removeClass('bongmo');
							$("#rowed5 tbody").find('input:checkbox').each(function(index, element) {
								 $("#"+element.id).prop('disabled',false);
							...
							var n_rs=data.split(';;');
							//alert(tam);
							$("#rowed5").jqGrid('setCell',tam,'BNChiTra', parseInt(n_rs[1])-parseInt(n_rs[4])-parseInt(n_rs[6]));
							...
							$("#rowed5").jqGrid('setCell',tam,'HoTro', n_rs[6]);
							if(n_rs[5]==0){
                ...
                {name: 'Xoa', index: 'Xoa', search: false, width: "7%", editable: false, align: 'left', hidden: false},
                {name: 'LoaiKham', index: 'LoaiKham', search: false, width: "35%", editable: false, align: 'left', hidden: false,edittype:"select",formatter:'select',editoptions:{value: tenloaikham},classes:'loai_kham',cellattr: function (rowId, val, rawObject) {
                	//var dataFromTheRow = jQuery('#rowed5').jqGrid ('getRowData', rowId);
                		//var tenbhyt=$('#rowed5').jqGrid('getCell',rowId,'TenBHYT');
                		if(rawObject['TenBHYT']){
                			tenbh=rawObject['TenBHYT'];
                		}else{
                			tenbh=rawObject[37];
				...
				{name: 'ThanhTien', index: 'ThanhTien', search: false, width: "14%", editable: false, align: 'right', hidden: true,formatter:'currency',formatoptions:{decimalSeparator:".", thousandsSeparator: ",", decimalPlaces: 0, prefix: "", defaultValue: '0'}},
				{name: 'DonGiaBHYT', index: 'DonGiaBHYT', search: false, width: "14%", editable: false, align: 'right', hidden: true,formatter:'currency',formatoptions:{decimalSeparator:".", thousandsSeparator: ",", decimalPlaces: 0, prefix: "", defaultValue: '0'}},
				{name: 'PhuThuBV', index: 'PhuThuBV', search: false, width: "14%", editable: false, align: 'right', hidden: true,formatter:'currency',formatoptions:{decimalSeparator:".", thousandsSeparator: ",", decimalPlaces: 0, prefix: "", defaultValue: '0'}},
				{name: 'SLMoiNgay', index: 'SLMoiNgay', search: false, width: "14%", editable: true,edittype:"text",formatter:'number',formatoptions:{decimalSeparator:".", thousandsSeparator: ",", decimalPlaces: 0, prefix: "", defaultValue: '1'},editoptions: { dataEvents: [{ type: 'keyup', fn: function(e) {
					var row = $(e.target).closest('tr.jqgrow');
					...
					setTimeout(function(){
						if((window.loaidoituongkham=="BHYT") && (parseInt(DonGiaBHYT)>0 )){
							...
							if(parseInt(phuthu[n1]["HoTro"])>0){
								var hotro=(parseInt(phuthu[n1]["HoTro"])*parseInt(sl_moi_ngay)) * parseInt(so_ngay);
								bn_chitra=phi_thuc_hien-hotro;
							}else{
								var hotro=0;
							}
							$('#rowed5').jqGrid('setCell',rowId,'PhiThucHien',phi_thuc_hien,'');
							$('#rowed5').jqGrid('setCell',rowId,'BNChiTra',bn_chitra,'');
							$('#rowed5').jqGrid('setCell',rowId,'HoTro',hotro,'');
...
, align: 'center', hidden: false,classes:'slmoingay'},
				{name: 'SoNgay', index: 'SoNgay', search: false, width: "14%", editable: true,edittype:"text",formatter:'number',formatoptions:{decimalSeparator:".", thousandsSeparator: ",", decimalPlaces: 0, prefix: "", defaultValue: '1'},editoptions: { dataEvents: [{ type: 'keyup change', fn: function(e) {
						var row = $(e.target).closest('tr.jqgrow');
						var rowId = row.attr('id');
						var sl_moi_ngay=$('#'+rowId+'_SLMoiNgay').val();
						var thanh_tien=$('#rowed5').jqGrid('getCell',rowId,'ThanhTien');
			     ...
			     {name: 'HoTro', index: 'HoTro', search: false, width: "14%", editable: false, align: 'right', hidden: false,formatter:'currency',formatoptions:{decimalSeparator:".", thousandsSeparator: ",", decimalPlaces: 0, prefix: "", defaultValue: '0'}},
				 {name: 'BNChiTra', index: 'BNChiTra', search: false, width: "15%", editable: false, align: 'right', hidden: false,formatter:'currency',formatoptions:{decimalSeparator:".", thousandsSeparator: ",", decimalPlaces: 0, prefix: "", defaultValue: '0'}},
                ...
                {name: 'ThucHien', index: 'ThucHien', search: false, width: "18%", editable: true, align: 'center', hidden: false,edittype:"select",formatter:'select',editoptions:{value:"0:Hospital;1:Home;2:Self",dataEvents: [{ type: 'change keyup', fn: function(e) {
						var tmp5 = $("#rowed5").jqGrid('getDataIDs'); 
						var row = $(e.target).closest('tr.jqgrow');
						var rowId = row.attr('id');
				...
				{name: 'NguoiChiDinh', index: 'NguoiChiDinh', search: false, width: "20%", editable: false, align: 'left', hidden: false,edittype:"select",formatter:'select',editoptions:{value: nguoichidinh} , classes:'nguoichidinh'},
				{name:'NguoiChiDinh2',index:'NguoiChiDinh2',width:"20%", align:'left',hidden:true, editable: true,edittype:"select", classes:'nguoichidinh2',editoptions:{value:nguoichidinh},formatter: "select"},
			{name: 'GiaThueNgoaiThucHien', index: 'GiaThueNgoaiThucHien', search: false, width: "30%", editable: false, align: 'left', hidden: true},
				{name: 'ThoiGianTrungBinhThucHien', index: 'ThoiGianTrungBinhThucHien', search: false, width: "30%", editable: false, align: 'left', hidden: true},
				...
				{name: 'Color', index: 'Color', search: false, width: "20%", editable: false, align: 'left', hidden: true},
				{name: 'Sua', index: 'Sua', search: false, width: "5%", editable: false, align: 'left', hidden: true},
				{name: 'IDBacSyCD', index: 'IDBacSyCD', search: false, width: "5%", editable: false, align: 'left', hidden: true},
				{name: 'Code', index: 'Code', search: false, width: "10%", editable: true, align: 'left', hidden: false,editoptions:{dataEvents: [{ type: 'change keyup', fn: function(e) { 
					var row = $(e.target).closest('tr.jqgrow');
					var rowId = row.attr('id');
					if($('#'+rowId+'_Code').val()!='' && isNaN($('#'+rowId+'_Code').val())===true){
						$('#'+rowId+'_Code').val('');
					}
					number_textbox('#'+rowId+'_Code');
					$("#rowed5").jqGrid('setCell',rowId,'Sua', '1');
				}}]}
				},
	...
	function create_goikham() {
        $("#data_goikham").jqGrid({
            ...
            colModel: [
                ...
                {name: 'TenGoiKham', index: 'TenGoiKham', search: true, width: "20%", editable: false, align: 'left', hidden: false},
				...
				{name: 'MoTa', index: 'MoTa', search: false, width: "10%", editable: false, align: 'left', hidden: true},
				{name: 'GhiChu', index: 'GhiChu', width: "30%", editable: false, align: 'left', hidden: true},
            ],
           		loadonce: true,
				scroll: 1,	 
				modal:true,	 	
	...
	function create_loaikham() {
        ...
        $("#data_loaikham").jqGrid({
            data: mydata,
            datatype: "local",
            colNames: ['Id gói khám','Tên loại khám','Giá báo BN', 'Giá báo cho Bệnh nhân','Đơn giá BHYT','Phụ thu', 'Mô tả',  'GiaThueNgoaiThucHien',  'ThoiGianTrungBinhThucHien',  'IDLuotKham',  'IDPhongChuyenMon',  'MaBenhNhan',  'ID_NhomCLS','','','','TenBHYT'],
            colModel: [
				{name: 'ID_LoaiKham', index: 'ID_LoaiKham', width: "10%", editable: false, align: 'left', hidden: true},
                {name: 'TenLoaiKham', index: 'TenLoaiKham', search: true, width: "20%", editable: false, align: 'left', hidden: false,cellattr: function (rowId, val, rawObject) {
				    return rawObject[16] === "" ? ' title="Tên BHYT: Không có"' : ' title="Tên BHYT: '+rawObject[16]+'"';
				}},
				...
				{name: 'GhiChu', index: 'GhiChu', search: false, width: "30%", editable: false, align: 'left', hidden: true},
				{name: 'GiaThueNgoaiThucHien', index: 'GiaThueNgoaiThucHien', search: false, width: "30%", editable: false, align: 'left', hidden: true},
				{name: 'ThoiGianTrungBinhThucHien', index: 'ThoiGianTrungBinhThucHien', search: false, width: "30%", editable: false, align: 'left', hidden: true},
				{name: 'IDLuotKham', index: 'IDLuotKham', search: false, width: "30%", editable: false, align: 'left', hidden: true},
				{name: 'IDPhongChuyenMon', index: 'IDPhongChuyenMon', search: false, width: "30%", editable: false, align: 'left', hidden: true},
				...
				var tmp3 = $("#data_loaikham").jqGrid('getDataIDs'); 
				 for(var i=0;i < tmp3.length;i++){ 
					...
					if(rowData["Color"]=="1"){
						...
						$("#data_loaikham").jqGrid('setRowData', tmp3[i], false, { background: '#DE5145',border:'1px solid #dfd9c3' });
					}
				 }
			},
            caption: "Chi tiết gói khám"
        });
        $("#data_goikham").jqGrid('navGrid', "#pdata_goikham", {add: false, edit: false, del: false, search: false, refresh: true, closeAfterEdit: true, clearAfterAdd: true, navkeys: [true, 38, 40], closeOnEscape: true});
	...
	if((datacheck=='0' && window.n_khoachidinh!=1) || window.chophepluu==1){
		...
		var tmp5 = $("#rowed5").jqGrid('getDataIDs'); 
		  ...
		  var phithuchien =0;
			...
			$.post('pages.php?module=<?= $modules ?>&view=<?= $view ?>&action=check_xoa_dtph&id='+_n_id+'&nhom='+rowData2["IDNhomCLS"]).done(function(data) {
					if(data=='true'){
						if(rowData2["IDNhomCLS"]==25 || rowData2["IDNhomCLS"]==26){
							$("#idk2").val(tam);
							$('#dialog2').dialog('open');
...
function load_select_tenloaikham(){
	//window.tenloaikham = $.ajax({url: "pages.php?module=web_services&function=get_auto_edit_option&action=index&term=&status=2&tables=DM_LoaiKham&id=ID_LoaiKham&name=TenLoaiKham", async: false, success: function(data, result) {if (!result) alert('Không load được danh mục loại khám');}}).responseText;	
	window.tenloaikham = $.ajax({url: "pages.php?module=<?= $modules ?>&view=<?= $view ?>&action=data_nhomttpt", async: false, success: function(data, result) {if (!result) alert('Không load được danh mục loại khám');}}).responseText;	
	
}
function load_select_phy(){
	window.phy = jQuery.parseJSON($.ajax({url: "pages.php?module=<?= $modules ?>&view=<?= $view ?>&action=data_nhomcon&ac=phy", async: false, success: function(data, result) {if (!result) alert('Không load được danh mục loại khám');}}).responseText);	
}
...
function fun_inchidinh(){
	$('#btn_inphieucd,#btn_incd').button('enable');
	...
	dialog_report("Xem trước khi in",90,90,"u78787878975f","pages.php?module=report&view=<?=$modules?>&action=phieuchidinhdtph_all&header=top&vatlytrilieu="+window.vatlytrilieu+"&dieuduong="+window.dieuduong+"&phauthuatgiaithuat="+window.phauthuatgiaithuat+"&family_planning="+window.family_planning+"&id_benhnhan="+$("#idbenhnhan").val()+"&id_luotkham="+$("#idluotkham").val()+"&type=report&id_form=10"+"&xemin="+window.xemin,'PhieuChiDinhDieuTri');
...
function chuyen(rowId){
	//alert(window.hotro_sum);
	//$.post('pages.php?module=<?= $modules ?>&view=<?= $view ?>&action=check_thanhtoan&idluotkham=<?=$_GET['idluotkham']?>').done(function(data){
	$.ajax({
	type: 'POST',
	async : false,
	url: 'pages.php?module=<?= $modules ?>&view=<?= $view ?>&action=check_thanhtoan&idluotkham=<?=$_GET['idluotkham']?>',
	success: function(data, status, xhr) {
		var datacheck=$.trim(data);
		if(datacheck=='0' && window.n_khoachidinh!=1){
		//  if(window.n_dathanhtoanbill!=1 && window.n_khoachidinh!=1){
			...
			type: 'POST',
			async : false,
			url: 'pages.php?module=<?= $modules ?>&view=<?= $view ?>&action=get_hotro&idluotkham=<?=$_GET['idluotkham']?>',
			success: function(data, status, xhr) {
				window.n_hotrobhyt=parseFloat(data);
				//console.log(window.n_hotrobhyt)
				 var tmp1 = $("#rowed5").jqGrid('getDataIDs'); 
				 var tam,ln=0;
				 var dem=tmp1+1;
				 ...
				 if(window.loaidoituongkham=="BHYT" && rowData["GiaTheoBaoHiem"]>0 && parseInt(rowData['Color'])!=2){
					// alert(rowData["PhanTramCungChiTra"]);
					var n_phantram =rowData["PhanTramCungChiTra"];
					var n_hotro=0;
					var n_tonghotro=0;
					...
					//alert(window.phantrammoi_new[0]+'__'+window.phantrammoi_new[1]);
					//alert(window.n_trangthaikham);
					...
					if(window.n_trangthaikham==3){// trai tuyen
						if(window.phantrammoi==100){
							var thanhtien_vienphi=rowData["GiaHienThi"];
							var thanhtien_bhyt=parseFloat(rowData["GiaTheoBaoHiem"]);
							var phi_thuchien=rowData["GiaHienThi"];
							var bn_chitra=rowData["GiaHienThi"];
							var bnchitrabh=rowData["GiaTheoBaoHiem"];
							...
							var bnchitrabh=((parseFloat(rowData["GiaTheoBaoHiem"])*window.phantrammoi)/100);
							var bh_chitra=parseFloat(rowData["GiaTheoBaoHiem"])-bnchitrabh;
							var bn_chitra=(parseFloat(rowData["GiaHienThi"])-bh_chitra);					
						}
					 }else{
						var thanhtien_vienphi=rowData["GiaBaoChoBN"];
						var thanhtien_bhyt=parseFloat(rowData["GiaTheoBaoHiem"]);
						...
						var bnchitrabh=((parseFloat(rowData["GiaTheoBaoHiem"])*window.phantram)/100);
						var bh_chitra=(rowData["GiaTheoBaoHiem"])-bnchitrabh;
						var bn_chitra=(parseFloat(rowData["GiaHienThi"])-bh_chitra);
						n_hotro=0;
						window.phantrammoi=window.phantram;
				...
				//alert(window.phantrammoi);
				n_tonghotro=parseFloat(window.n_hotrobhyt)+parseFloat(window.hotro_sum)+parseInt(rowData['HoTro']);
				if(n_tonghotro<400000 && window.loaidoituongkham=="BHYT"){
					if(n_tonghotro>=200000){
						alert("Lượt khám này đã hỗ trợ BHYT >=200.000đ.");
					}
					if(n_tonghotro>0){
						var thanhtien_vienphi=rowData["GiaHienThi"];
					...
					if(parseInt(rowData['HoTro'])>0){
						bn_chitra=bn_chitra-parseInt(rowData['HoTro']);
						n_hotro=parseInt(rowData['HoTro']);
					}
				}else if(window.loaidoituongkham=="BHYT"){// khong ho tro
					alert("Lượt khám này đã hỗ trợ BHYT >=400.000đ nên không được hỗ trợ thêm, vui lòng làm bệnh án nội trú bằng cách (nhấn vào nút phiếu vào viện) trong form bệnh án.");
					var thanhtien_vienphi=rowData["GiaHienThi"];
					var thanhtien_bhyt=parseFloat(rowData["GiaTheoBaoHiem"]);
					var phi_thuchien=rowData["GiaHienThi"];
				...
				if(n_hotro>0 || window.phantrammoi<100)
					var isbh=1;
				else
					var isbh=0;
					
				//alert(window.phantram_luu);
					parameters =
							{
								initdata : {IDLoaiKham:rowData["ID_LoaiKham"],Xoa:"Xóa",LoaiKham:rowData["ID_LoaiKham"],ThanhTien:thanhtien_vienphi,PhiThucHien:phi_thuchien,TrangThai:"DangCho",ThucHien: "0",NguoiChiDinh:"<?=$_SESSION["user"]["id_user"] ?>",NguoiChiDinh2:"<?=$_SESSION["user"]["id_user"] ?>",GiaThueNgoaiThucHien:rowData["GiaThueNgoaiThucHien"],ThoiGianTrungBinhThucHien:rowData["ThoiGianTrungBinhThucHien"],IDLuotKham:$("#idluotkham").val(),IDPhongChuyenMon:$("#idphongbanchuyenmon").val(),MaBenhNhan:$("#mabenhnhan").val(),IDNhomCLS:rowData["ID_NhomCLS"],DonGiaBHYT:rowData['GiaTheoBaoHiem'],PhuThuBV:rowData["GiaPhuThu"],ThanhTienBaoHiem:bh_chitra,BNChiTra:bn_chitra,PhanTramCungChiTra:window.phantram_luu,ThanhTienCungChiTra:bnchitrabh,Color:rowData["Color"],HoTro:n_hotro,IsBHYT:isbh,TenBHYT:rowData["TenBHYT"]},
								position :"last",
								useDefValues : false,
								useFormatter : false,
								addRowParams :{
										...
										oneditfunc: function(rowId) {	
											...
											//inline_enter(rowId);
											if(isbh==1)
												$("#"+rowId+"_FixGiaBHYT").prop('checked',true);
										},	
										afterrestorefunc: function(rowId) {	
											destroy_combobox_inline_new('NguoiChiDinh2',rowId,'rowed5');					
											$("#rowed5").jqGrid('bindKeys');
				...
				if(dem==1){
					if(rowData["NhomBHYT"]==8 && window.loaidoituongkham=="BHYT" && window.n_maloaidt=="GD_TESTTHOIMA" && window.n_songaythe<150){
						...
						if(strconfirm_gd == true){
							...
							parameters2 =
							{
								initdata : {IDLoaiKham:rowData["ID_LoaiKham"],Xoa:"Xóa",LoaiKham:rowData["ID_LoaiKham"],ThanhTien:thanhtien_vienphi,PhiThucHien:phi_thuchien,TrangThai:"DangCho",ThucHien: "0",NguoiChiDinh:"<?=$_SESSION["user"]["id_user"] ?>",NguoiChiDinh2:"<?=$_SESSION["user"]["id_user"] ?>",GiaThueNgoaiThucHien:rowData["GiaThueNgoaiThucHien"],ThoiGianTrungBinhThucHien:rowData["ThoiGianTrungBinhThucHien"],IDLuotKham:$("#idluotkham").val(),IDPhongChuyenMon:$("#idphongbanchuyenmon").val(),MaBenhNhan:$("#mabenhnhan").val(),IDNhomCLS:rowData["ID_NhomCLS"],DonGiaBHYT:rowData['GiaTheoBaoHiem'],PhuThuBV:rowData["GiaPhuThu"],ThanhTienBaoHiem:bh_chitra,BNChiTra:bn_chitra,PhanTramCungChiTra:window.phantram_luu2,ThanhTienCungChiTra:bnchitrabh,Color:rowData["Color"]},
								position :"last",
								useDefValues : false,
								useFormatter : false,
								addRowParams :{
...
function resize_control() {
	...
	$("#rowed5").setGridHeight($(".right_col").height() - 135);
