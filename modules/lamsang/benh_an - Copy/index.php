...
<datalist id="ylenh">
  ...
  <option value="Lấy thuốc đem về">
            ...
            <input id="ngaytaikham" name="ngaytaikham"   type="text" style="width:60px;" >
...
$('#taikham').click(function(){
	if($('#taikham').is(':checked')){
        ...
        $("#ngaytaikham").val(convert_date_to_string2(NgayHetThuoc.addDays(1)));			
    }else{
...
function convert_date_to_string2(inputA){
thoigian=$.datepicker.formatDate('dd/mm/yy', inputA);
	return thoigian;
}
...
window.user_login=<?=$_SESSION["user"]["id_user"]?>;
	...
	if(window.user_login==21 && <?=$_GET['id_benhnhan']?>==350){// Nam thêm account để có quyền chuyển bs chính
...
function create_lichsu_donthuoc(){	
	...
	$("#rowed6").jqGrid({	
		...
		{name:'PhuongThucThucHien',index:'PhuongThucThucHien',width:"40%",align:'center',hidden:true, editable: false,edittype:"select",formatter:'select',editoptions:{value:"0:Hospital;1:Home;2:Self", dataEvents:  [{ type: 'blur', fn: function(e) { 
			savedRow = $('#rowed6').jqGrid("getGridParam", "savedRow");					 
			if (savedRow && savedRow.length > 0) {		
				if($("#rowed6").find('.jqgrid-new-row').length==false)	{ 	  
					$('#rowed6').jqGrid("saveCell", savedRow[0].id, savedRow[0].ic);			
		...
		onCellSelect: function (rowid,iCol,cellcontent,e) {			
			...
			if((iCol==5)&&(rowData['ID_trangthai']=='DangKham'||rowData['ID_trangthai']=='DangSua'||rowData['ID_trangthai']=='DaThucHien')&&(user_login==rowData['BSChanDoan'])){
				$.post('pages.php?module=<?=$modules?>&view=<?=$view?>&action=controller_donthuoc&id='+rowData1['iddonthuocct']+'&oper=del&ID_LuotKham='+$("#rowed3").getRowData(rowed3_select)['ID_LuotKham']).done(function(data){
		...
		caption: "<?php lang('dthuoc')?> (Insert để thêm dòng)",
