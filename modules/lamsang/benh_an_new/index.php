    ...
    <div class="ui-widget-content ui-layout-east"> 
     <div class="thongtin_dieutri">
        ...
        <div id="prowed7" style="display:none"></div>       
         ...
         <div style="">
  		    <input class="custom_button xanh" type="button"  id="chitietquota" value="<?php lang('ctietqt')?>" style="height:22px;width:80px!important;display:none">
            <input class="custom_button xanh" type="button" id="miengiam"  value="<?php lang('miengiam')?>" style="height:22px;width:60px!important;display:none">
            <input class="custom_button xanh" type="button"  value="<?php lang('gchudduong')?>" style="height:22px;width:100px!important;display:none">
            <a href="javascript:void(0)" class="custom_button xanh" id="quota" style="color: #0057da;margin-left:5px;display:none">Quota (<span id="view_quota">0</span>)</a>
            <span style="font-size:14px;color:#36F;right:0px;position:absolute;" id="tongtien"></span>
        </div>
            <div  style="padding-left:5px;">
            <label for="toanmau"  style="width:150px;text-align:left"><?php lang('toamau')?> </label>
...
$('#btn_batdau,#btn_chidinhkham,#btn_dieutriphoihop,#btn_hoantat,#btn_luu,#btn_lammoi,#btn_dantoa,#btn_saotoa,#btn_xemtoa,#btn_intoa,#btn_chinhsua,#btn_phieuchuyen,#btn_phieu,#btn_benhannoitru,#btn_vltl,#btn_ngtru,#btn_innhiet,#btn_xeminnhiet').button();
...
window.user_login=<?=$_SESSION["user"]["id_user"]?>;
	if(window.user_login==785){// Nam thêm account Mr Nam ngày 12/9/16 để có quyền chuyển bs chính
		window.IsDoctor=1;
	}
...
function create_lichsu_donthuoc(){	
	...
	$("#rowed6").jqGrid({	
		data:mydata,	 
		datatype: "local",		 	
		colNames:['','','','','BHYT','','<?php lang('tenthuoc')?>',"<?php lang('giaban')?>", "<?php lang('ddung')?>", "<?php lang('sl')?>", "<?php lang('tong')?>", "Dược xuất", "<?php lang('cdung')?>", "<?php lang('thhien')?>"],
		colModel:[		
		...
		{name:'isbhyt',index:'isbhyt', width:"5%",editoptions: { value:"1:0"}, editable:true,align:'center',hidden:false,edittype:"checkbox",formatter:"checkbox",formatoptions:{disabled: false}},
	...
	,newgrid:true,func_grid:"check_soluong",editoptions:{
		...
		{name:'CachDungLieuDung',index:'CachDungLieuDung',width:"30%",align:'left',hidden:false, editable: true
	...
	{name:'PhuongThucThucHien',index:'PhuongThucThucHien',width:"7%",align:'center',hidden:true, editable: true,edittype:"select",formatter:'select',editoptions:{value:"0:Hospital;1:Home;2:Self", dataEvents:  [{ type: 'blur', fn: function(e) { 
		savedRow = $('#rowed6').jqGrid("getGridParam", "savedRow");					 
		if (savedRow && savedRow.length > 0) {		
			if($("#rowed6").find('.jqgrid-new-row').length==false)	{ 	  
				$('#rowed6').jqGrid("saveCell", savedRow[0].id, savedRow[0].ic);			
		...
		onCellSelect: function (rowid,iCol,cellcontent,e) {			
			...
			if((iCol==5)&&(rowData['ID_trangthai']=='DangKham'||rowData['ID_trangthai']=='DangSua'||rowData['ID_trangthai']=='DaThucHien')&&(user_login==rowData['BSChanDoan'])){
				...
				$.post('pages.php?module=<?=$modules?>&view=<?=$view?>&action=controller_donthuoc&id='+rowData1['iddonthuocct']+'&oper=del').done(function(data){
		...
		loadComplete: function(data) {
			...
			if($("#rowed6").getGridParam("reccount")-($($("#rowed6").find('.jqgrid-new-row')).length)>0){
				...
				if (rowdata6['trangthaidonthuoc']=='Cancel'){	
					...
					$("#rowed6").setGridHeight($(".ui-layout-east").height()-250); 
				}
			...
			$.post('pages.php?module=<?=$modules?>&view=<?=$view?>&action=data_chung&ID_LuotKham='+rowData["ID_LuotKham"]).done(function(data){
				...
				$.each( datacls, function( key, val ) {		 
					for(i=0;i<val.length;i++){
						...
						}else if(tam1_cls[2]==4556||tam1_cls[2]==4383||tam1_cls[2]==6750||tam1_cls[2]==6751){
