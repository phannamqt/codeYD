...
switch ($_GET["oper"]) {
    ...
    update_datraKQ();
    break;
     case "update_SoLanGoiLoa":
    update_SoLanGoiLoa();
    break;
}
function update_SoLanGoiLoa(){

    $data= new SQLServer;//tao lop ket noi SQL
    $store_name="{call GD2_AddSoLanGoiLoa (?,?,?,?)}";//tao bien khai bao store
       
    $params = array($_GET['id_luotkham'],null,$_SERVER['REMOTE_ADDR'],$_SESSION["user"]["id_user"]);
        
    $data->query( $store_name, $params);//Goi store
   // echo ($_GET['Dem'].'--id_luotkham--'.$_GET['id_luotkham']);
