...
$khoa=CheckUpdate_BhytBhccThanhtoan(NULL,$_GET['id_kham'],NULL,NULL,$_SESSION["user"]["id_user"]);
	if($khoa['Isupdate']==0){
		...
		if($_GET['ac']==1){// la bao hiem
			...
			if($ketqua<>2){
				$store_name="{call MED_Kham_FixGiaBHYT (?,?,?,?,?)}";
			...
			7;;".$ketqua.";;";
*/
			echo json_encode(array('IsLock' => 0, 'Notes' => 'Đã lưu', 'PhiThucHien' => $tam[0]['PhiThucHien'], 'PhanTramCungChiTra' =>$tam[0]['PhanTramCungChiTra'], 'ThanhTienCungChiTra' =>$tam[0]['ThanhTienCungChiTra'], 'ThanhTienBaoHiem' =>$tam[0]['ThanhTienBaoHiem'], 'Isbhyt' =>$tam[0]['Isbhyt'], 'HoTro' =>$tam[0]['HoTro'], 'KetQua' =>$ketqua, 'PhiDuKien' => $tam[0]['PhiDuKien'], 'ThanhTienPhuThuBenhVien' => $tam[0]['ThanhTienPhuThuBenhVien']));
}
