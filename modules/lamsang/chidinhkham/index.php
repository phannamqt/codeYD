		...
		$('#dialog').dialog({
                ...
                buttons: {
                    "Ok": function() {
						...
						$("#rowed5").jqGrid('setRowData',$("#idk").val(),{DonGiaVienPhi:0});
						$("#rowed5").jqGrid('setRowData',$("#idk").val(),{DonGiaBHYT:0});
						
						$("#rowed5").jqGrid('setRowData',$("#idk").val(),{PhuThu:0});
						$("#rowed5").jqGrid('setRowData',$("#idk").val(),{BHChiTra:0});
...
function create_grid2(){
	...
	$("#rowed4").jqGrid({
		data: mydata,
		datatype: "local",
		colNames: ['GiaThueNgoaiThucHien', 'ThoiGianTrungBinhThucHien', 'ID_DMLoaiKham_Phongban','ID_LoaiKham', '<?php lang('tenlk')?>', '<?php lang('giavp')?>', '<?php lang('giavp')?>','<?php lang('giabhyt')?>','Phụ thu','P.Thu tại nhà','Hệ số người nước ngoài','<?php lang('bhyttra')?>','Color','','','<?php lang('hotro')?>','TenBHYT'],
		colModel: [
        ...
        $("#rowed5").jqGrid({
			...
			,editoptions: { value:"1:0",dataEvents:  [{ type: 'click', fn: function(e) { 
			   ...
			   if(IDKham==''){
						...
						$("#"+$(e.target).attr('id')).prop('checked',true);
							...
							$.post('pages.php?module=<?= $modules ?>&view=<?= $view ?>&action=controller_fix_bhyt&hienmaloi=a&id_kham='+tam+'&ac='+tthai).done(function(data) {
								...
								if(data.IsLock==1){
									...
									$("#rowed5").jqGrid('setCell',tam,'ThanhTienVienPhi', parseInt(data.PhiDuKien))
									...
									$("#rowed5").jqGrid('setCell',tam,'PhuThu', parseInt(data.ThanhTienPhuThuBenhVien));
									if(data.Isbhyt==0){
                ...
                {name: 'DonGiaVienPhi', index: 'DonGiaVienPhi', search: false, width: "12%", editable: false, align: 'right', hidden: false,formatter:'currency',formatoptions:{decimalSeparator:".", thousandsSeparator: ",", decimalPlaces: 0, prefix: "", defaultValue: '0'}},
				...
				{name: 'HoTro', index: 'HoTro', search: false, width: "9%", editable: false, align: 'right', hidden: true,formatter:'currency',formatoptions:{decimalSeparator:".", thousandsSeparator: ",", decimalPlaces: 0, prefix: "", defaultValue: '0'}},
                ...
                {name: 'PhiThucHien', index: 'PhiThucHien', search: false, width: "9%", editable: false, align: 'right', hidden: true,formatter:'currency',formatoptions:{decimalSeparator:".", thousandsSeparator: ",", decimalPlaces: 0, prefix: "", defaultValue: '0'}},
...
function tinhtongtien(){
	...
	var  Sum_DonGiaVienPhi= $("#rowed5").jqGrid('getCol', 'DonGiaVienPhi', false, 'sum');
	var  Sum_DonGiaBHYT = $("#rowed5").jqGrid('getCol', 'DonGiaBHYT', false, 'sum');
	var  Sum_PhuThu = $("#rowed5").jqGrid('getCol', 'PhuThu', false, 'sum');
	...
	$("#rowed5").jqGrid('footerData','set', {
		...
		HoTro:Sum_HoTro,
		DonGiaVienPhi:Sum_DonGiaVienPhi,
...
function ChuyenCD(GiaBHYT,PhanTramCungChiTra,GiaHienThi,HoTro,ID_LoaiKham,TenLoaiKham,GiaBaoChoBN,GiaPhuThu,GiaThueNgoaiThucHien,IDPhongChuyenMon,MaBenhNhan,ID_NhomCLS,NhomThucHien,ThoiGianTrungBinhThucHien,Color,TenBHYT,NhomBHYT){
	//GiaBaoChoBN=parseFloat(GiaBaoChoBN)+parseFloat(GiaPhuThu);
