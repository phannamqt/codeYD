...
Vui lòng chọn một lượt khám để thực hiện chức năng này!!!
 ...
 <div  class="data_chongoikham" title="Chọn gói khám" style="display:none">
  ...
  <div  class="xemtoathuoc" title="Xem toa thuốc">
   ...
   <input type="hidden" id="idluotkham" value="<?php if(isset($_GET['idluotkham'])){ echo $_GET['idluotkham'];} ?>" /> 
	...
	<div class="ui-layout-west ui-widget-content left_col">   <table id="rowed3" ></table></div>
        ...
        <a  id="miengiam" style="margin-left: 5px;;display:none"  href="#" > Miễn giảm </a>
		<span id="thongbao" style="margin-left: 30px; font-weight:bold; color:#FF0000;"> </span>
        </div>
        <a  style="display:none"  id="xemtoathuoc" style="margin-left: 5px;"  href="#" > Xem toa thuốc </a>
        <a  style="display:none"  id="chongoikham" class="fm-button ui-state-default ui-corner-all "  > Chọn gói khám</a>
...
$('#dialog').dialog({
	...
	buttons: {
		"Ok": function() {
			...
			$("#rowed5").jqGrid('setRowData',$("#idk").val(),{DonGiaBHYT:0});
			$("#rowed5").jqGrid('setRowData',$("#idk").val(),{PhuThuBV:0});
...
var lastsel;
    ...
    function create_grid2(){
        ...
        $("#rowed4").jqGrid({
            data: mydata,
            datatype: "local",
            colNames: ['GiaThueNgoaiThucHien', 'ThoiGianTrungBinhThucHien', 'ID_LoaiKham', 'Tên loại khám', 'Đ.Giá Viện phí', 'Đ.Giá Viện phí','Đ.Giá BHYT','Phụ thu','P.Thu tại nhà','Hệ số người nước ngoài','BH trả','','ID_NhomCLS','','','Hỗ trợ','TenBHYT'],
            colModel: [
...
function create_grid3(){
        ...
        $("#rowed5").jqGrid({
			...
			,editoptions: { value:"1:0",dataEvents:  [{ type: 'click', fn: function(e) { 
				...
				{name: 'ThanhTien', index: 'ThanhTien', search: false, width: "14%", editable: false, align: 'right', hidden: false,formatter:'currency',formatoptions:{decimalSeparator:".", thousandsSeparator: ",", decimalPlaces: 0, prefix: "", defaultValue: '0'}},
				{name: 'DonGiaBHYT', index: 'DonGiaBHYT', search: false, width: "14%", editable: false, align: 'right', hidden: false,formatter:'currency',formatoptions:{decimalSeparator:".", thousandsSeparator: ",", decimalPlaces: 0, prefix: "", defaultValue: '0'}},
				{name: 'PhuThuBV', index: 'PhuThuBV', search: false, width: "14%", editable: false, align: 'right', hidden: false,formatter:'currency',formatoptions:{decimalSeparator:".", thousandsSeparator: ",", decimalPlaces: 0, prefix: "", defaultValue: '0'}},
				{name: 'SLMoiNgay', index: 'SLMoiNgay', search: false, width: "14%", editable: true,edittype:"text",formatter:'number',formatoptions:{decimalSeparator:".", thousandsSeparator: ",", decimalPlaces: 0, prefix: "", defaultValue: '1'},editoptions: { dataEvents: [{ type: 'keyup', fn: function(e) {
					...
					var thanh_tien=parseInt($('#rowed5').jqGrid('getCell',rowId,'ThanhTien'))+parseInt($('#rowed5').jqGrid('getCell',rowId,'PhuThuBV'));
					...
					var n1=0; 
						if((window.loaidoituongkham=="BHYT") && (parseInt(DonGiaBHYT)>0 )){
							...
							var thanhtien_bhyt=0;
							if(n_thuchien==0){
								var phi_thuchien=thanh_tien;
								var thanhtien_vienphi=thanh_tien;
							}else if(n_thuchien==1){
								var phi_thuchien=parseInt(thanh_tien);
								var thanhtien_vienphi=thanh_tien;
							}else{
								var phi_thuchien=0;
...
, align: 'center', hidden: false,classes:'slmoingay'},
				{name: 'SoNgay', index: 'SoNgay', search: false, width: "14%", editable: true,edittype:"text",formatter:'number',formatoptions:{decimalSeparator:".", thousandsSeparator: ",", decimalPlaces: 0, prefix: "", defaultValue: '1'},editoptions: { dataEvents: [{ type: 'keyup change', fn: function(e) {
			     ...
			     {name: 'HoTro', index: 'HoTro', search: false, width: "14%", editable: false, align: 'right', hidden: true,formatter:'currency',formatoptions:{decimalSeparator:".", thousandsSeparator: ",", decimalPlaces: 0, prefix: "", defaultValue: '0'}},
                ...
                {name: 'ThucHien', index: 'ThucHien', search: false, width: "18%", editable: true, align: 'center', hidden: true,edittype:"select",formatter:'select',editoptions:{value:"0:Hospital;1:Home;2:Self",dataEvents: [{ type: 'change keyup', fn: function(e) {
						var tmp5 = $("#rowed5").jqGrid('getDataIDs'); 
						var row = $(e.target).closest('tr.jqgrow');
						var rowId = row.attr('id');
			...
			{name: 'GiaThueNgoaiThucHien', index: 'GiaThueNgoaiThucHien', search: false, width: "30%", editable: false, align: 'left', hidden: true},
				...
				{name: 'Code', index: 'Code', search: false, width: "10%", editable: true, align: 'left', hidden: true,editoptions:{dataEvents: [{ type: 'change keyup', fn: function(e) { 
					var row = $(e.target).closest('tr.jqgrow');
					var rowId = row.attr('id');
	...
	function create_loaikham() {
        ...
        $("#data_loaikham").jqGrid({
            data: mydata,
            datatype: "local",
            colNames: ['Id gói khám','Tên loại khám','Giá báo cho Bệnh nhân', 'Giá báo cho Bệnh nhân','Đơn giá BHYT','Phụ thu', 'Mô tả',  'GiaThueNgoaiThucHien',  'ThoiGianTrungBinhThucHien',  'IDLuotKham',  'IDPhongChuyenMon',  'MaBenhNhan',  'ID_NhomCLS','','','','TenBHYT'],
            colModel: [
				{name: 'ID_LoaiKham', index: 'ID_LoaiKham', width: "10%", editable: false, align: 'left', hidden: true},
                {name: 'TenLoaiKham', index: 'TenLoaiKham', search: false, width: "20%", editable: false, align: 'left', hidden: false,cellattr: function (rowId, val, rawObject) {
...
function tinhtongtien(){
	...
	var  Sum_ThanhTien = $("#rowed5").jqGrid('getCol', 'ThanhTien', false, 'sum');
	var  Sum_DonGiaBHYT = $("#rowed5").jqGrid('getCol', 'DonGiaBHYT', false, 'sum');
	var  Sum_PhuThuBV = $("#rowed5").jqGrid('getCol', 'PhuThuBV', false, 'sum');
	...
	$("#rowed5").jqGrid('footerData','set', {
		...
		DonGiaBHYT:Sum_DonGiaBHYT,
		PhuThuBV:Sum_PhuThuBV
	});
}
...
function chuyen(rowId){
	...
	success: function(data, status, xhr) {
		...
		//  if(window.n_dathanhtoanbill!=1 && window.n_khoachidinh!=1){
			...
			success: function(data, status, xhr) {
				window.n_hotrobhyt=0;//parseFloat(data);
				//console.log(window.n_hotrobhyt)
				 ...
				 if(window.loaidoituongkham=="BHYT" && rowData["GiaTheoBaoHiem"]>0 && parseInt(rowData['Color'])!=2){
					...
					var n_phantram ="0.0";//rowData["PhanTramCungChiTra"];
					var n_hotro=0;
					var n_tonghotro=0;
