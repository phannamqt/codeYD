...
$data= new SQLServer;	
	...
	if(isset($_REQUEST['ID_Thuoc'])){
		...
		$store_name2="{call MED_KiemTraTonKhoByXML(?)}";
		$get2=$data->query( $store_name2,$params2);
		$excute2= new SQLServerResult($get2);
		$tam2= $excute2->get_as_array();
		 
		if(count($tam2)>0){
			$params2 = array('<DsThuoc><Thuoc><ID_Thuoc>'.$_REQUEST['ID_Thuoc'].'</ID_Thuoc><SoLuongCanXuat>'.$_REQUEST['ThanhTien'].'</SoLuongCanXuat><ID_Kho>3</ID_Kho></Thuoc></DsThuoc>');
			$store_name2="{call MED_KiemTraTonKhoByXML(?)}";
			$get2=$data->query( $store_name2,$params2);
			$excute2= new SQLServerResult($get2);
			$tam2= $excute2->get_as_array();
	...
	if($oper=='add'){	
		...
		if(substr($_POST['CachDungLieuDung'],0,50)=='<input role="textbox" style="width: 82%;" name="Ca'){
			...
			$Gia=$_POST['Gia'];
			$SoThuocThucXuat=convert_comma_donthuoc($_POST['SoThuocThucXuat']);
			$store_name1="{call MED_donthuocchitiet_upd_benhan(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)}";
