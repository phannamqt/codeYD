			...
			$.post('pages.php?module=<?=$modules?>&view=<?=$view?>&action=data_chung&ID_LuotKham='+rowData["ID_LuotKham"]).done(function(data){
				...
				$.each( datacls, function( key, val ) {		 
					for(i=0;i<val.length;i++){
						...
						}else if(tam1_cls[2]==4556||tam1_cls[2]==4383||tam1_cls[2]==6750||tam1_cls[2]==9835 ||tam1_cls[4]=='KSKCT' ){
...
function create_thuocmau(elem){	
	...
	$(elem).jqGrid({
		...
		{name:'CachDungCT',index:'CachDungCT'},	
		],
		hidegrid: false,
		gridview: true,
		loadonce: true,
