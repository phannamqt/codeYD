...
$table = "DM_KhachHangLaCTy";
$Id_key = 'ID_KhachHangCTy';
...
foreach ($tam as $row) {//duyet toan bo mang tra ve
    ...
    $responce->rows[$i]['cell']=array($row["TenCty"],$row["MaVietTat"],$row["DiaChi"],$row["DienThoai"],$row["Email"],
...
$store_name="{call GD2_DM_form_select()}";//tao bien khai bao store
...
$i=0;
foreach ($tam as $row) {//duyet toan bo mang tra ve
    $responce->rows[$i]['id']=$row["ID_Form"];
    $responce->rows[$i]['cell']=array($row["ID_Form"],$row["TenForm"],$row["TenDienGiai"],$row["Active"],$row["STT"] );
