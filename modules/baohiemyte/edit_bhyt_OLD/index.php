...
$(document).ready(function() {
	window.socket = io.connect('192.168.1.15:3000');
...
function create_luotkham(elem){
	$(elem).jqGrid({
		data:[],
		datatype: "local",
		colNames:[ 'BHYT','Phân loại','Mã thẻ','Mã KCB','Ngày vào','Thanh toán','Tổng ngày','ID lượt khám','Xuất XML','Xuất XML tay','In bảng kê'],
		colModel:[
			{name:'LoaiDoiTuongKham',index:'LoaiDoiTuongKham', editable:false,hidden:true,hidden:true},  
...
function create_cls(elem) {
	$(elem).jqGrid({
		...
		groupingView : {
			...
			groupSummary:false,
			groupText : ['<b>{0} - {1} : {GiaBaoHiem} | {ThanhTienBaoHiem}</b>']
		},
...
{ 	if(parseInt(id)>0){
		...
		alert("Dữ liệu sẽ được xuất vào folder server 1.15: ../VAS/4210");
	}else{
...
{ 	if(parseInt(id)>0){
		...
		alert("Dữ liệu sẽ được xuất vào folder server 1.15: ../VAS/XuatTay");
	}else{
		alert("Lượt này chưa thanh toán nên không thể xuất dữ liệu!");
	}
}
