...
<?php 
$data= new SQLServer;
if($_GET['mabenhan']=='add'){
	$store_name="{call GD2_thongtin_phieuthu (?)}";
...
$khoa=CheckUpdate_KhoaHoSo(0,$_GET["idbenhnhan"],$_SESSION["user"]["id_user"]);
...
<td>
<input id="thungan" name="thungan" style="width:98%;"type="text" value="<?php if($_GET['mabenhan']=='add'){echo $_SESSION["user"]["nickname"];}else{echo $thongtin[0]['NickName']; }?>">  
</td>
<td>
...
<tr>
<td  align="right">
<label for="tienhuycd" style="width:150px; " id="tienhuycd_label"></label>
</td>
<td >
...
$(document).ready(function() {	
	...
	window.oper='<?php if($_GET['mabenhan']=='add'){ echo  'add';}else{echo 'edit';}?>';
	...
	create_grid1();
	resize_control();
	$('#nocuoi').val((<?php echo  $thongtin[0]['nocu'];?>).formatMoney(0, ',', '.'));
	...
	$('#id_print').keydown(function(e){
		if (jwerty.is("enter",e)) {	
			if($('#id_print').val()==4){
				...
				dialog_report("Xem trước khi in",90,90,"u78787878975f","pages.php?module=report&view=<?=$modules?>&action=phieu_thu&header=top&lien="+$('#id_print').val()+"&type=report&tenreport=1&id_form=10&idthutrano="+window.ID_ThuTraNo,'PhieuThu');
				$('#dialog_print').dialog('close');
			}
	...
	$('#btn_lien1').click(function(){
		...
		dialog_report("Xem trước khi in",90,90,"u78787878975f","pages.php?module=report&view=<?=$modules?>&action=phieu_thu&header=top&tenreport=1&lien=2&type=report&id_form=10&idthutrano="+window.ID_ThuTraNo,'PhieuThu');
		$('#dialog_print').dialog('close');	   
	})
	$('#btn_lien2').click(function(){
		$.cookie("in_status", "print_preview"); 
		dialog_report("Xem trước khi in",90,90,"u78787878975f","pages.php?module=report&view=<?=$modules?>&action=phieu_thu&header=top&tenreport=1&lien=3&type=report&id_form=10&idthutrano="+window.ID_ThuTraNo,'PhieuThu');
		$('#dialog_print').dialog('close');	   
	})
	phanquyen();
	<?php
		if($khoa['Ma']==6){		
			echo "$('#btn_luu,#btn_lien1,#btn_lien2,#btn_sua,#btn_huy').button('disable');";
			echo "$('#tienthu').prop('disabled', true);";
			echo "alert('".$khoa['Chuoi']."')";	
		}
	?>
})
function luu(){
	if(dem==0){
		...
		$.post('pages.php?module=<?=$modules?>&view=<?=$view?>&action=controller&hienmaloi=1',datatosend).done(function(data){
			window.ID_ThuTraNo=$.trim(data);
			 window.oper='edit';
			trangthai(oper);
			 $("#rowed4").jqGrid('setGridParam',{url: 'pages.php?module=<?= $modules ?>&view=<?= $view ?>&action=data_chitietcongno&id_benhnhan='+id_benhnhan,datatype:'json'}).trigger('reloadGrid');
		})
	}
