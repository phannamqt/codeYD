...
?>
 ...
 <?php 
			...
			$data= new SQLServer;
			$store_name="{call GD2_HuyChiDinhThuoc_HoanTien (?)}";
...
setTimeout(function(){$("#tienthu").focus()},500);
   ...
   $('#btn_luu').click(function(){  
    	...
    	$('#dialog_sotien').html('Số tiền thu là <b>'+$('#tienthu').val()+'</b>. Bạn có muốn tiếp tục. <br><i style="color:red">(Vui lòng kiểm tra kỹ trước khi hoàn tiền. Các phiếu đã hoàn tiền IT sẽ không thể can thiệp để hủy hay sửa.)</i>');
	...
	if(dem==0){
	    ...
	    $.post('pages.php?module=<?=$modules?>&view=<?=$view?>&action=controller&hienmaloi=1',datatosend).done(function(data){
		   window.ID_ThuTraNo=data;
		    tooltip_message('Đã hoàn ứng và nhập kho');
	   })
	}
