...
<fieldset style="display:inline-block;margin-top:7px;height:180px;width:400px" id="mid">
  <div class="form_row">
   ...
   <input lang='luu' name="bhyt" style="width:100px;height:15px;display:none" type="text" value="0" id="bhyt" disabled>
   ...
   <label style="font-weight:bold;font-size:10px;width:100px;display:inline-block">K/H Thuốc VTYT</label>
   <input lang='luu' name="KhauHaoThuocVTYT" style="width:100px;height:15px" type="text" value="0" id="KhauHaoThuocVTYT" disabled>

   <label style="font-weight:bold;font-size:12px;width:70px;display:inline-block">K/H Dịch Vụ</label>
   <input lang='luu' name="KhauHaoDichVu" style="width:100px;height:15px" type="text" id="KhauHaoDichVu" disabled>
   <br>
...
<div >	 <button id="btn_refesh3" >Thêm template</button>
  ...
  <button href="#" id="btn_bhyt" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only" style="margin-left:0px;  margin-bottom:1px;display:none ">Bảng kê BHYT</button>
...
$(document).ready(function() {
  ...
  $('#btn_bhyt').click(function(){
   $.cookie("in_status", "print_preview");
   dialog_report("Xem trước khi in",90,90,"u78787878975f","pages.php?module=report&view=<?=$modules?>&action=bhyt&header=top&lien=2&type=report&id_form=10&idthutrano="+ID_ThuTraNo,'ChitietBHYT');
   $('#dialog_print').dialog('close');
 })
...
function create_grid() {
  ...
  $("#rowed3").jqGrid({
    url: url_rowed3,
    datatype: "json",
    colNames: ['ID_LoaiKham','ID_Kham','Tên loại khám','Trạng thái','Phí thực hiện','Giá bảo hiểm'
    ,'Số lần','Phần trăm','BHYT chi trả','Cùng chi trả','Hỗ trợ BV','BHCC trả','KH Thuôc/VTYT','KH DV','Miễn giảm','Tổng phải trả'
    ,'Tiền vốn','Tên nhóm','','','','','','','','',''],
    colModel: [			
    ...
    {name: 'Phantram', index: 'Phantram', search: false, width: "5%", editable: false, align: 'left', hidden: VP}, 
    ...
    {name: 'KhauHaoThuocVTYT', index: 'KhauHaoThuocVTYT', search: false, width: "15%", editable: false, align: 'right', hidden: false,formatter:"integer"},
    {name: 'KhauHaoDichVu', index: 'KhauHaoDichVu', search: false, width: "15%", editable: false, align: 'right', hidden: false,formatter:"integer"},

    {name: 'miengiam', index: 'miengiam', search: false, width: "15%", editable: false, align: 'right', hidden: false,formatter:"integer"},				
...
function tinhlai(){
	...
	//nam	 miengiam
	window.mg_tongphaitra=tongcong-(sum_bhyt_tra+sum_bhcc_tra+hotro_thuoc+hotro_kham);
	...
	$('#hotro').val((hotro_thuoc+hotro_kham).formatMoney(0, ',', '.'));
	if(window.oper=='edit'){
...
if(!VP && TrangThaiKham!=3 && oper=='add' && tongbhyt<=181500){			     
 var ids_rowed3 = $("#rowed3").getDataIDs();
 for(var i=0;i<ids_rowed3.length;i++){	
   bhyt_tam=$('#rowed3').jqGrid('getCell', ids_rowed3[i], 'bhyt');
   Cungchitra_tam=$('#rowed3').jqGrid('getCell', ids_rowed3[i], 'Cungchitra');	
