...
editoptions: { dataEvents:  [{ type: 'keyup', fn: function(e) { 
	if(jwerty.is("enter",e)||jwerty.is("tab",e)){
		if(!check_lohandung($(e.target).attr('id').split('_')[0])){
			...
			$.post('pages.php?module=<?= $modules ?>&view=<?= $view ?>&action=controller&oper=checksolo&hienmaloi=a&SoLoNhaSanXuat='+$("#"+$(e.target).attr('id').split('_')[0]+"_SoLoNhaSanXuat").val()).done(function(data) {
				data=data.split("|");
				if(parseInt(data[1])>0){
					alert("Thuốc này có lô NSX và hạn dùng trùng với thuốc trước,hãy thêm dấu _(gạch dưới)!");
					$("#"+$(e.target).attr('id').split('_')[0]+"_SoLoNhaSanXuat").val("");
					$("#"+$(e.target).attr('id').split('_')[0]+"_SoLoNhaSanXuat").focus();
					$("#"+$(e.target).attr('id').split('_')[0]+"_SoLoNhaSanXuat").select();
				}else{
