...
jQuery(document).ready(function() {
  $("#dialog-huychot").dialog({
    ...
    buttons: {
      YES() {
        $(this).dialog("close");
        $.post(
          `${urlHome}&action=huychot&id_phieu=${window.n_id_huychot}`
        ).done(function(data) {
          $("#xem").click();
  ...
  $("#excel_noibo").click(function(e) {
    window.open(
      `resource.php?module=report&view=quanlyduoc&action=PhieuXuatNoiBo_Excel&type=excel&tungay=${$(
        "#tungay"
      ).val()}&denngay=${$("#denngay").val()}`
    );
  });
  $("#excel_noibokhoapong").click(function(e) {
    // $("#tenkhoahienthi").html($("#khoa").val());
    // $("#rowed6").jqGrid('setGridParam',{datatype: 'json',url:"${urlHome}&action=data_danhsach_phieuxuatnoibo_tonghop&tungay="+$("#tungay").val()+"&denngay="+$("#denngay").val()+"&khoa="+$("#khoa_hidden").val()}).trigger('reloadGrid');
    window.open(
      `resource.php?module=report&view=quanlyduoc&action=PhieuXuatNoiBoKhoaPhong_Excel&type=excel&tungay=${$(
        "#tungay"
      ).val()}&denngay=${$("#denngay").val()}&khoa=${$(
        "#khoa_hidden"
      ).val()}&khoahienthi=${$("#khoa").val()}`
    );
  });
  $("#gopphieu").click(function(e) {
    ...
    if (demgop > 1) {
      $.post(
        `${urlHome}&action=controller_gopphieu&id=${window.id_gopphieu}`
      ).done(function(data) {
        $("#xem").click();
      });
    } else {
  ...
  $("#huygop").click(function(e) {
    $.post(`${urlHome}&action=controller_huygop&id=${window.maphieugop}`).done(
      function(data) {
        $("#xem").click();
      }
    );
  ...
  $("#dialog-duyetxuat").dialog({
    ...
    buttons: {
      YES() {
        ...
        $.post(
          `${urlHome}&action=controller_duyetxuat&hienmaloi=a&id_phieu=${window.idxuatnoibo}&maphieugop=${window._maphieugop}&idkho=${window.n_idkhoxuat}`
        )
          .done()
          .done(function(data) {
            data = $.parseJSON(data);
  ...
  $("#xemtonghop").click(function(e) {
    ...
    $("#rowed6")
      .jqGrid("setGridParam", {
        datatype: "json",
        url: `${urlHome}&action=data_danhsach_phieuxuatnoibo_tonghop&tungay=${$(
          "#tungay"
        ).val()}&denngay=${$("#denngay").val()}&khoa=${$("#khoa_hidden").val()}`
      })
      .trigger("reloadGrid");
    // alert();
  });
  ...
  $("#xem").click(function(e) {
    ...
    if (window.n_action == 1) {
      $("#rowed3")
        .jqGrid("setGridParam", {
          datatype: "json",
          url: `${urlHome}&action=data_danhsach_phieuxuatnoibo&tungay=${$(
            "#tungay"
          ).val()}&denngay=${$("#denngay").val()}`
        })
        .trigger("reloadGrid");
      $("#rowed4")
        .jqGrid("setGridParam", {
          datatype: "json",
          url: `${urlHome}&action=data_danhsach_phieuxuatnoibo_daduyet&tungay=${$(
            "#tungay"
          ).val()}&denngay=${$("#denngay").val()}`
        })
        .trigger("reloadGrid");
      ...
      $("#rowed33")
        .jqGrid("setGridParam", {
          datatype: "json",
          url: `${urlHome}&action=data_danhsach_phieuxuatdieuchuyen&tungay=${$(
            "#tungay"
          ).val()}&denngay=${$("#denngay").val()}`
        })
        .trigger("reloadGrid");
      $("#rowed44")
        .jqGrid("setGridParam", {
          datatype: "json",
          url: `${urlHome}&action=data_danhsach_phieuxuatdieuchuyen_daduyet&tungay=${$(
            "#tungay"
          ).val()}&denngay=${$("#denngay").val()}`
        })
        .trigger("reloadGrid");
  ...
  $("#xemin").click(function(e) {
    ...
    $.post(
      `${urlHome}&action=checkchot&id_phieuxuatnoibo=${window.inphieuid}`
    ).done(function(data) {
      if (data == "true") {
        ...
        if (window.n_action == 1) {
          if (window.dangchonluoi == 1) {
            dialog_report(
              ...
              90,
              "u78787878975f",
              `resource.php?module=report&view=quanlyduoc&action=phieuxuatthuocnoibo&idphieu=${window.inphieuid}&type=report&id_form=10`,
              "PhieuLinhThuoc"
            );
          } else {
            dialog_report(
              ...
              90,
              "u78787878975f",
              `resource.php?module=report&view=quanlyduoc&action=phieuxuatthuocnoibo_daxuat&idphieu=${window.inphieuid}&type=report&id_form=10`,
              "PhieuLinhThuoc"
            );
          }
        } else if (window.dangchonluoi == 1) {
          dialog_report(
            "Xem trước khi in",
            90,
            90,
            "u78787878975f",
            `resource.php?module=report&view=quanlyduoc&action=phieuxuatdieuchuyen_new&idphieu=${window.inphieuid}&maphieugop=&type=report&id_form=10`,
            "PhieuLinhThuoc"
          );
        } else {
          dialog_report(
            ...
            90,
            "u78787878975f",
            `resource.php?module=report&view=quanlyduoc&action=phieuxuatdieuchuyen_daxuat&idphieu=${window.inphieuid}&maphieugop=&type=report&id_form=10`,
            "PhieuLinhThuoc"
          );
        }
      } else {
    ...
    $.post(
      ...
      if (data == "true") {
        ...
        if (window.n_action == 1) {
          ...
          $("body").append(
            `<div id="dialog_edit_tienhd" style="display:none">
                ...
                click(e) {
                  ...
                  if (window.dangchonluoi == 1) {
                    dialog_report(
                      ...
                      90,
                      "u78787878975f",
                      `resource.php?module=report&view=quanlyduoc&action=phieuxuatthuocnoibo_print_23&chuky=1&idphieu=${window.inphieuid}&ngay_edit=${ngayin}&type=report&id_form=10`,
                      "PhieuLinhThuoc"
                    );
                  } else {
                    dialog_report(
                      ...
                      90,
                      "u78787878975f",
                      `resource.php?module=report&view=quanlyduoc&action=phieuxuatthuocnoibo_daxuat_kt&chuky=1&idphieu=${window.inphieuid}&ngay_edit=${ngayin}&type=report&id_form=10`,
                      "PhieuLinhThuoc"
                    );
                  }
                  $("#dialog_edit_tienhd").dialog("close");
  ...
  $("#xeminphieugop").click(function(e) {
    dialog_report(
      ...
      90,
      "u78787878975f",
      `resource.php?module=report&view=quanlyduoc&action=phieuxuatdieuchuyen_new&idphieu=${window.maphieugop}&maphieugop=${window.maphieugop}&type=report&id_form=10`,
      "PhieuLinhThuoc"
    );
  });
        ...
        if (window.n_action == 1) {
          if (window.dangchonluoi == 1) {
            dialog_report(
              ...
              90,
              "u78787878975f",
              `resource.php?module=report&view=quanlyduoc&action=phieuxuatthuocnoibo_innhiet&idphieu=${window.inphieuid}&type=report&id_form=10`,
              "PhieuChiDinh"
            );
          } else {
            dialog_report(
              ...
              90,
              "u78787878975f",
              `resource.php?module=report&view=quanlyduoc&action=phieuxuatthuocnoibo_innhiet_daxuat&idphieu=${window.inphieuid}&type=report&id_form=10`,
              "PhieuChiDinh"
            );
          }
        } else {
          ...
          if (window.dangchonluoi == 1) {
            dialog_report(
              ...
              90,
              "u78787878975f",
              `resource.php?module=report&view=quanlyduoc&action=phieuxuatdieuchuyen_innhiet&idphieu=${window.inphieuid}&maphieugop=&type=report&id_form=10`,
              "PhieuChiDinh"
            );
          } else {
            dialog_report(
              ...
              90,
              "u78787878975f",
              `resource.php?module=report&view=quanlyduoc&action=phieuxuatdieuchuyen_innhiet_daxuat&idphieu=${window.inphieuid}&maphieugop=&type=report&id_form=10`,
              "PhieuChiDinh"
            );
          }
        }
    ...
    buttons: {
      YES: _.debounce(
        function() {
          ...
          if (window.n_action == 1) {
            $.post(
              `${urlHome}&action=controller&hienmaloi=a&idxuatnoibo=${window.idxuatnoibo}&nguoitao=${window.idnguoitao}`
            ).done(function(data) {
              data = $.parseJSON(data);
              if (data.check == 0) {
                tooltip_message("Duyệt thành công");
          ...
          } else if (window._maphieugop > 0) {
            $.post(
              `${urlHome}&action=controller_dieuchuyen_gopphieu&hienmaloi=a&maphieugop=${window._maphieugop}&tukho=${window.tukho}&denkho=${window.denkho}&nguoitao=${window.idnguoitao}&nguoiduyetxuat=${window.idnguoiduyetxuat}`
            ).done(function(data) {
              data = data.split(";");
              if (data[2] == 1) {
            ...
            // $.post(
            //   '${urlHome}&action=controller_dieuchuyen&hienmaloi=a&idxuatnoibo=' +
            //   window.idxuatnoibo + '&tukho=' + window.tukho + '&denkho=' + window.denkho + '&nguoitao=' +
            ...
            //   }
            // });
            var urlTam = "http://192.168.1.104:3001/xuatdieuchuyen";
            $.ajax({
              type: "POST",
              async: false,
              data: {
...
function create_grid() {
  $("#rowed3").jqGrid({
    ...
    colModel: [
          ...
          dataEvents: [
            {
              ...
              fn(e) {
                ...
                $.post(
                  `${urlHome}&action=controller_bhyt&id=${tam}&v=${_val}`
                ).done(function(data) {
                  tooltip_message("Lưu thành công");
                });
              }
      ...
      {
        name: "ID_PhieuXuatNoiBo",
        index: "ID_PhieuXuatNoiBo",
        width: "5%",
        editable: false,
        stype: "text",
        search: true,
    ...
    onSelectRow(id) {
      ...
      $("#rowed5")
        .jqGrid("setGridParam", {
          datatype: "json",
          url: `${urlHome}&action=data_danhsach_phieuxuatnoibochitiet&id_phieuxuatnoibo=${id}&maphieugop=`
        })
        .trigger("reloadGrid");
    },
    ondblClickRow(rowId, rowIndex, columnIndex) {
      ...
      $.post(`${urlHome}&action=checkchot&id_phieuxuatnoibo=${rowId}`).done(
        function(data) {
          if (data == "false") {
            var para = `resource.php?module=quanlyduoc&view=phieu_xuat_noibo_new&id_form=1679&idphieu=${rowId}&idphongban=${dataRow.ID_Khoa}&chotphieu=${dataRow.ChotPhieu}&nguoitao=${dataRow.IDNguoiTao}&ac=1`;
            dialog_main(
              `Chỉnh sửa phiếu xuất nội bộ số phiếu: ${dataRow.SoPhieu}`,
              90,
              90,
              "56743265743657",
              para
            ...
            $("#noidungchotphieu").html(
              "Phiếu này đã chốt nên không được phép sửa"
            );
            $("#dialog-loichotphieu").dialog("open");
          }
        }
    ...
    caption:
      "Ds phiếu xuất nội bộ (Chưa duyệt) <div class='grid1 '><div class='hinhvuong chuachot'></div><label class='diengiai'>Chưa chốt</label></div>"
  });
  ...
  $("#rowed3").jqGrid("navButtonAdd", "#prowed3", {
    ...
    onClickButton() {
      ...
      $.post(
        `${urlHome}&action=checkchot&id_phieuxuatnoibo=${window.idxuatnoibo}`
      ).done(function(data) {
        if (data == "true") {
          $("#dialog-confirm").dialog("open");
        } else {
          $("#noidungchotphieu").html(
            `Phiếu này ${dataRow2.Khoa} chưa chốt nên không thể duyệt`
          );
          $("#dialog-loichotphieu").dialog("open");
          // tooltip_message("Phiếu này "+dataRow2['Khoa']+" chưa chốt nên không thể duyệt");
        }
  ...
  $("#rowed3").jqGrid("navButtonAdd", "#prowed3", {
    ...
    onClickButton() {
      var para =
        "resource.php?module=quanlyduoc&view=phieu_xuat_noibo_new&id_form=1679";
...
function create_grid_2() {
  $("#rowed33").jqGrid({
    ...
    onSelectRow(id) {
      ...
      if (dataRow.DuyetXuat == "") {
        ...
        if (
          permission[`kho_${ntamtukho}`] != "" &&
          permission[`kho_${ntamtukho}`] == 1
        ) {
        ...
        if (
          permission[`kho_${ntamdenkho}`] != "" &&
          permission[`kho_${ntamdenkho}`] == 1
        ) {
          $("#rowed33_duyetnhap").removeClass("ui-state-disabled");
        } else {
          $("#rowed33_duyetnhap").addClass("ui-state-disabled");
      ...
      $("#rowed5")
        .jqGrid("setGridParam", {
          datatype: "json",
          url: `${urlHome}&action=data_danhsach_phieuxuatnoibochitiet&id_phieuxuatnoibo=${id}&maphieugop=${dataRow.MaPhieuGop}`
        })
        .trigger("reloadGrid");
    },
    ondblClickRow(rowId, rowIndex, columnIndex) {
      ...
      $.post(`${urlHome}&action=checkchot&id_phieuxuatnoibo=${rowId}`).done(
        function(data) {
          if (data == "false") {
            var para = `resource.php?module=quanlyduoc&view=phieu_dieuchuyen_new&id_form=1679&idphieu=${rowId}&chotphieu=${dataRow.ChotPhieu}&tukho=${dataRow.ID_TuKho}&denkho=${dataRow.ID_DenKho}&nguoitao=${dataRow.IDNguoiTao}&ac=2`;
            dialog_main(
              `Chỉnh sửa phiếu xuất điều chuyển số phiếu: ${dataRow.SoPhieu}`,
              90,
              90,
              "56743265743658",
              para
    ...
    caption:
      "Ds phiếu xuất điều chuyển (Chưa duyệt) <div class='grid1 '><div class='hinhvuong chuachot'></div><label class='diengiai'>Chưa chốt</label></div> <span style='margin-left:100px'>Nếu đã duyệt xuất vẫn không thấy nút <span style='color:red'>Duyệt nhập kho</span> vui lòng nhấn nút <span style='color:red'>Xem</span></span>"
  });
  ...
  $("#rowed33").jqGrid("navButtonAdd", "#prowed33", {
    ...
    onClickButton() {
      var dataRow2 = jQuery("#rowed33").jqGrid(
        "getRowData",
        window.idxuatnoibo
      );
      ...
      $.post(
        ...
        if (data == "true") {
          ...
          $("#noidungchotphieu").html(
            "Phiếu này chưa chốt nên không thể duyệt"
          );
          $("#dialog-loichotphieu").dialog("open");
          // tooltip_message("Phiếu này "+dataRow2['Khoa']+" chưa chốt nên không thể duyệt");
        }
  ...
  $("#rowed33").jqGrid("navButtonAdd", "#prowed33", {
    ...
    onClickButton() {
      var para =
        "resource.php?module=quanlyduoc&view=phieu_dieuchuyen_new&id_form=1679";
      dialog_main(
        "Thêm mới phiếu xuất điều chuyển",
        90,
        90,
        "56743265743658",
        para
...
function create_khoa(elem, datalocal) {
  ...
  $(elem).jqGrid({
    ...
    onSelectRow(id) {},
    ondblClickRow(id, rowIndex, columnIndex) {},
    loadComplete(data) {}
  });
...
function resize_control() {
  ...
  // $('#rowed55').setGridWidth($('.n_tren').width() - 10);
  // $('#rowed55').setGridHeight($('.n_tren').height() - 113);
