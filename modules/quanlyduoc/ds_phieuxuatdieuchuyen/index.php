...
<body>
  ...
  <div id="dialog-loithuoc" title="Thông báo tồn kho" style="display:none">
    <p><span id="tenthuoc_ton" style="font-weight:bold;"></span></p>
	<p><span style="font-weight:bold; color: red">Số lượng tồn không đủ để xuất</span> </p>
  </div>
  <div id='n_top_menu' style="margin-top:5px;display:inline-block; padding-top:5px;">
    ...
    <span style='margin-left:10px'>Từ ngày<input type='text' id='tungay'
        value='<?php echo date("d/m/Y")?>' style=' width:70px; text-align:center'>
      đến ngày <input type='text' id='denngay' value='<?php echo date("d/m/Y")?>'
        style=' width:70px; text-align:center'> 
	<button id='xem' style='margin-left:5px;0.2em 1em !important; margin-top: -4px;'> Xem</button></span>
...
function create_grid_2() {
  $("#rowed33").jqGrid({
      ...
      $.post('resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=checkchot&id_phieuxuatnoibo=' + rowId)
        .done(function(data) {
          if (data == 'false') {
            ...
            //tooltip_message("Phiếu này đã chốt nên không được phép sửa");
            $("#noidungchotphieu").html('Phiếu này đã khóa nên không được phép sửa');
            $("#dialog-loichotphieu").dialog('open');
          }
        });
    },
    ...
    caption: "Ds phiếu xuất điều chuyển (Chưa duyệt)"
  });
  ...
  $("#rowed33").jqGrid('navButtonAdd', '#prowed33', {
    ...
    onClickButton: function() {
      ...
      window._maphieugop = dataRow2['MaPhieuGop'];
	   if (permission['kho_' + window.tukho] != '' && permission['kho_' + window.tukho] == 1) {
          $("#dialog-duyetxuat").dialog('open');
        } else {
          $("#noidungchotphieu").html("Bạn không được cấp quyền duyệt xuất từ kho "+dataRow2['KhoXuat']+"!!!");
            $("#dialog-loichotphieu").dialog('open');
        }
...
function create_grid3() {
  ...
  $("#rowed5").jqGrid({
    data: mydata,
    datatype: "local",
    colNames: ['Tên thuốc', 'ĐVT', 'SL', 'TenNhom'],
    colModel: [{
        name: 'TenGoc',
        index: 'TenGoc',
        search: true,
