 ...
 <?php include("module_SlickGrid.php"); ?>  
<style type="text/css">
#rowed3 td,
#rowed4 td,
#rowed5 td {
...
#gridPhieuNhapTraNoiBo,#gridPhieuNhapTraNoiBoChiTiet{
...
<body>
  ...
  <div id='n_top_menu' style="margin-top:5px;display:inline-flex; padding-top:5px;">
    ...
    <input type="radio" id="tranoibo" name="chon"><label for="tranoibo">P.trả N.bộ</label>
    ...
    <button id="xemin_ketoan" style='margin-left:5px;0.2em 1em !important; margin-top: -4px;'>In phiếu KT</button>
    ...
    <button id="excel_noibo" style='margin-left:30px;0.2em 1em !important; margin-top: -4px;'>Excel N.bộ</button>
    <button id="excel_noibokhoapong" style='margin-left:10px;0.2em 1em !important; margin-top: -4px;'>Excel</button>    
    <button id="xemin_xtnoibo" style='margin-left:10px;0.2em 1em !important; margin-top: -4px;'>In X.Trả N.Bộ</button>
  </div>
  <div id="panel_main">
  ...
  <div id="panel_main1" style="display:none;height:95%">

    <table  width='100%' height='95%'>
      <tr>
       <td valign="top" width="60%" height='95%' style="border-right: 1px solid #ccc;" >  
          <div class="grid-header" id="headerPhieuNhapTraNoiBo" style="width:100%">
            <label>Phiếu trả nội bộ</label>
          </div>
          <div id="gridPhieuNhapTraNoiBo"  style='height:90%;width:100%'></div>
          <div id="pagerPhieuNhapTraNoiBo" style='width:100%;height:20px;'></div>
       
        </td>
        <td valign="top"height='95%'>
        <div class="grid-header" id="headerPhieuNhapTraNoiBoChiTiet"  style="width:100%">
            <label>Chi tiết Phiếu trả nội bộ</label>
          </div>
          <div id="gridPhieuNhapTraNoiBoChiTiet"  style='height:90%;width:100%'></div>
          <div id="pagerPhieuNhapTraNoiBoChiTiet" style='width:100%;height:20px;'></div>
      ...
      <tr>
        <td >    
          <div>
            <button id='btnThemMoiTraNoiBo'>Thêm mới</button>            
            <button id='btnXoaTraNoiBo'>Xóa</button>
            <button id='btnChotTraNoiBo'>Chốt</button>
          </div>
        </td>
        <td>    
          <div>
            <button id='btnThemMoiTraNoiBoChiTiet'>Thêm mới</button>
            <button id='btnXoaTraNoiBoChiTiet'>Xóa</button>
          </div>
        </td>
      </tr>
    </table>  
...
window.id_user=<?=$_SESSION['user']['id_user']?>;
window.id_phongban=<?=$_SESSION['user']['id_phongban']?>;

var dialogPhieuNhapTraLai;
var tableNamePhieuNhapTraLai='Gd2_PhieuNhapXuat_TrungGian';
var idNamePhieuNhapTraLai='ID_PhieuTrungGian';
var gridPhieuNhapTraLai;
var dialogChiTietPhieuNhap;
var storeNameChiTietPhieuNhap='Gd2_PhieuNhapXuat_TrungGian_select_byidLoaiPhieu'
var dataChiTietPhieuNhap=function(){
  var from_day=convertDateSql($("#tungay").datepicker('getDate'));
	var to_day=convertDateSql($("#denngay").datepicker('getDate'))+' 23:59' 
  return  [from_day, to_day, 99];
}
window.n_action = 1;

var dialogPhieuNhapTraLaiChiTiet;
var tableNamePhieuNhapTraLaiChiTiet='Gd2_PhieuNhapXuat_TrungGian_ChiTiet';
var idNamePhieuNhapTraLaiChiTiet='ID_PhieuChiTiet';
var gridPhieuNhapTraLaiChiTiet;
var dialogChiTietPhieuNhapChiTiet;
var storeNameChiTietPhieuNhapChiTiet='Gd2_phieunhapxuat_trunggian_chitiet_byidphieu'
var dataChiTietPhieuNhapChiTiet=function(){
  if(!gridPhieuNhapTraLai.getSelectedData()){
    return [-1]
  }

  return  [gridPhieuNhapTraLai.getSelectedData().ID_PhieuTrungGian];
}

function XML5(row, cell, value, columnDef, dataContext) {
  return `<input type="button" onclick="XML5Dialog('${value}','${dataContext.ID_Thuoc}','${dataContext.TenGoc}');" value="Chọn" style="height:22px;width:50px;" class="custom_button xanh">`;
}
function XML5Dialog(ID_XuatKhoChiTiet,ID_Thuoc,TenGoc){
  dialogPhieuNhapTraLaiChiTiet.open();
  $(`#${dialogPhieuNhapTraLaiChiTiet.getUid()}_TenThuoc`).val(TenGoc);
  $(`#${dialogPhieuNhapTraLaiChiTiet.getUid()}_ID_PhieuTrungGian`).val(gridPhieuNhapTraLai.getSelectedData().ID_PhieuTrungGian);
  $(`#${dialogPhieuNhapTraLaiChiTiet.getUid()}_ID_Thuoc`).val(ID_Thuoc);
  $(`#${dialogPhieuNhapTraLaiChiTiet.getUid()}_ID_XuatKhoChiTiet`).val(ID_XuatKhoChiTiet);
}
...
//           } else {
//             // $.post(
//             //   'resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=controller_dieuchuyen&hienmaloi=a&idxuatnoibo=' +
...
<script type="text/javascript" src="modules/<?= $modules ?>/<?= $view ?>/file.js?q=<?=rand(1,10000)?>"></script>
<script type="text/javascript" src="modules/<?= $modules ?>/<?= $view ?>/fileTraNoiBo.js?q=<?=rand(1,10000)?>"></script>
<script type="text/javascript" src="modules/<?= $modules ?>/<?= $view ?>/fileTraNoiBoChiTiet.js?q=<?=rand(1,10000)?>"></script>
