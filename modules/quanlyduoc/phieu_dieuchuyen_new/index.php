...
function create_grid(){	
 var searhicon = '<span class="ui-state-error ui-state-hover  " style="border:0"><span class="ui-icon  ui-icon-closethick" style="float: left; margin-right: .3em;"></span></span>';			
	 $("#rowed3").jqGrid({				 
		...
		//url:'',
		datatype: "json",		
		colNames:['','Tên thuốc','ĐVT','Số lượng','Số lô nhà sản xuất','Ngày hết hạn', "luu"],
		colModel:[	
			...
			{name:'DonViTinh',index:'DonViTinh',search:false, width:"8%",editrules: {required: false}, editable:false,align:'left',hidden:false,edittype:"text"}, 	
			...
			,editrules:{custom: true,required:true, custom_func: function(value, colName) {
                           ...
                           return check_soluong(value,colName,id_row);
							...
							if(jwerty.is("enter",e)||jwerty.is("tab",e) || code == 13){
				 ...
				 {name:'SoLo',index:'SoLo',width:"8%",align:'left',hidden:false, editable: false
			,editoptions: { dataEvents:  [{ type: 'keyup', fn: function(e) { 
					if(jwerty.is("enter",e)||jwerty.is("tab",e)){
						
					}
                 } }]}},					
				 {name:'Ngayhethan',index:'Ngayhethan',width:"8%",align:'left',hidden:false, editable: false
			,editoptions: { 
...
$( "#dialog-form" ).dialog({
	...
	$("#rowed3").jqGrid('inlineNav', '#grid_footer', {add: true, addtext: 'Thêm thuốc', edittext: '', edit: false, save:true,
            addParams: {
				keys:true,
                position: "last",
                addRowParams: {
					...
					oneditfunc: function(rowId) {	
						...
						if($("#tukho").val()!=''){
							...
							tooltip_message("Vui lòng chọn kho trước");
							
							setTimeout(function(){ 
								$("#rowed3_iladd").removeClass('ui-state-disabled'); 
								$('#rowed3').jqGrid('delRowData',$('#rowed3').jqGrid("getGridParam", "selrow"));
							},50); 		
						}
					},	
		 ...
		 $("#rowed3").jqGrid('navButtonAdd', '#grid_footer', {caption: "Lưu [F10]", buttonicon: 'ui-icon-disk',id:'rowed3_luu',
            onClickButton: function() {
				if($("#tukho").val()!='' && $("#denkho").val()!=''){
					...
					tooltip_message("Vui lòng chọn kho!!!");
				}
            },
            title: "Lưu [F10]",
            position: "last"
...
function create_Dm_thuoc_grid(elem,datalocal){	
	...
	datastr:datalocal,	
	datatype: "jsonstring" ,
	colNames:['Tên thuốc','Nhóm thuốc','', 'Nước sản xuất','Nhà sản xuất','Hoạt chất','SignNumber','Đơn vị tính', 'Quy cách',"Số lô NSX", "Ngày hết hạn"],
	colModel:[			
		{name:'TenGoc',index:'TenGoc',width:"15%"},	
		{name:'TenNhomThuoc',index:'TenNhomThuoc',width:"10%"},		 
		{name:'MaThuoc',index:'MaThuoc',hidden :true},		
		{name:'TenDayDu',index:'TenDayDu',width:"10%"},	
		{name:'TenNhaSanXuat',index:'TenNhaSanXuat',width:"10%"},	 	 
		{name:'HoatChatChinh',index:'HoatChatChinh',width:"10%"},
		{name:'SignNumber',index:'SignNumber',hidden :true,width:"10%"},
		{name:'TenDonViTinh',index:'TenDonViTinh',hidden :false,width:"7%"},			
		{name:'QuyCach',index:'QuyCach',hidden :false,width:"10%"},
		{name:'SoLoNhaSanXuat',index:'SoLoNhaSanXuat',hidden :false,width:"7%"},
	...
	onSelectRow: function(id){	
		...
		$('#rowed3').jqGrid("setCell", $("#rowed3").jqGrid('getGridParam', 'selrow'), "SoLo",rowData['SoLoNhaSanXuat'] );
		$('#rowed3').jqGrid("setCell", $("#rowed3").jqGrid('getGridParam', 'selrow'), "Ngayhethan",rowData['NgayHetHan'] );
...
function create_tukho(elem, datalocal) {
        ...
        $(elem).jqGrid({
            ...
            onSelectRow: function(id) {
				...
				$.get('resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_DMthuoc&id_kho='+id).done(function(data) {
