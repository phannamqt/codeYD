...
<div id="dialog-themmoi" title="Bán kính" style="display:none">
	<table id="rowed6" >
	</table>
	<div id="prowed6" ></div> 
</div>
...
jQuery(document).ready(function() { 
	window.ID_LuotKham=0;			
	window.id_donthuoc=0;
	window.id_benhnhan=0;
	window.id_kham=0;
...
function create_sub_grid(){	 
	$("#rowed4").jqGrid({
		data:[],		
		datatype: "local",	
		colNames:['Mã','Tên','ĐVT', "S.lượng", "Đ.giá", "Thành tiền"],
		colModel:[		
...
function create_lichsu_donthuoc(){	
	...
	$("#rowed6").jqGrid({	
		data:mydata,	 
		datatype: "local",		 	
		colNames:['','','','','<?php lang('bh')?>','Xóa','<?php lang('tenthuoc')?>',"<?php lang('giaban')?>", "<?php lang('ddung')?>", "Số lượng", "<?php lang('tong')?> SL", "Dược xuất", "<?php lang('cdung')?>", "<?php lang('thhien')?>", "Cách dùng chi tiết"],
		colModel:[		
		...
		{name:'ID_DuongDungThuoc',index:'ID_DuongDungThuoc', width:"50%",align:'left',hidden:true, editable: true,edittype:"select",editoptions:{value:window.duongdung, dataEvents:  [{ type: 'keydown blur', fn: function(e) { 
		...
		{name:'ThanhTien',index:'ThanhTien',width:"40%",align:'right',hidden:true,formatter:'number'
		...
		{name:'CachDungLieuDung', index:'CachDungLieuDung',width:"50%",align:'right',hidden:true, editable: true
		...
		{name:'PhuongThucThucHien',index:'PhuongThucThucHien',width:"40%",align:'center',hidden:true, editable: true,edittype:"select",formatter:'select',editoptions:{value:"0:Hospital;1:Home;2:Self", dataEvents:  [{ type: 'blur', fn: function(e) { 
			savedRow = $('#rowed6').jqGrid("getGridParam", "savedRow");					 
			if (savedRow && savedRow.length > 0) {		
				if($("#rowed6").find('.jqgrid-new-row').length==false)	{ 	  
					$('#rowed6').jqGrid("saveCell", savedRow[0].id, savedRow[0].ic);			
		...
		{name:'CachDung',index:'CachDung',width:"550%",align:'left',hidden:true, editable: true,editoptions:{dataEvents:  [{ type: 'blur', fn: function(e) { 
			savedRow = $('#rowed6').jqGrid("getGridParam", "savedRow");					 
			if (savedRow && savedRow.length > 0) {		
				if($("#rowed6").find('.jqgrid-new-row').length==false)	{ 	  
					$('#rowed6').jqGrid("saveCell", savedRow[0].id, savedRow[0].ic);			
...
function check_product_available(mathuoc,tenthuoc,id_thuoc,tendonvitinh){	
	...
	for(i=0;i<id_rowed6.length;i++){		
		...
		if(rowData['ID_Thuoc']==id_thuoc){
			tooltip_message(tenthuoc+" đã có trên phiếu này");
			kiemtra=true;			 
		}			   
	}	 
	if(kiemtra==false){	 	 
...
if($.inArray(value, ids_rowed6) > -1){			 
	return [false,'đã có','ID_Thuoc1']
}else if($.trim(value)==''){
	return [false,'Chưa chọn','ID_Thuoc1']
}else{
	return[true,'']
}
