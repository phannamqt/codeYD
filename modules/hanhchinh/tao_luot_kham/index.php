...
<div>
  ...
  <!--   <select id="trangthai"  name="trangthai" style="width:200px;display:none" >
	...
	<div class="n-bhyt" style="display:inline-block;margin-left:0px!important;">
		 <fieldset>
               <legend>Kết quả thông tin truy vấn từ cổng BHYT</legend>
			    <div class="bhyt_info" id="bhyt_info" style="width:525px;height:328px;vertical-align:top;overflow: auto;"></div>
...
if(isset($_GET['idbhyt'])){
	...
	if(!isset($_GET["oper"])){
		...
		if($_GET["oper"]=='add'){
			...
			$get1=$data->query( "{call MED_lichhenkham_byidbenhnhan(?)}", $params1);
			$excute= new SQLServerResult($get1);
			...
			echo 'window.HoTenBenhNhan="'.$tam2[0]['HoTenBenhNhan'].'";'; 
			echo 'window.NgaySinhBenhNhan="'.($tam2[0]['NgayThangNamSinh']==Null?"":$tam2[0]['NgayThangNamSinh']->format('d/m/Y')).'";';	
			echo 'window.GioiTinhBenhNhan="'.$tam2[0]['GioiTinh'].'";';	
			...
			echo 'var ThoiGianVaoKham="'.($tam[0]['ThoiGianVaoKham']==Null?"":$tam[0]['ThoiGianVaoKham']->format('d/m/y H:i')).'";';	
			echo '$("#ngay").val("'.($tam[0]['ThoiGianVaoKham']==Null?"":$tam[0]['ThoiGianVaoKham']->format('d/m/Y H:i')).'");';
			...
			echo 'window.HoTenBenhNhan="'.$tam[0]['HoTenBenhNhan'].'";'; 
			echo 'window.NgaySinhBenhNhan="'.($tam[0]['NgayThangNamSinh']==Null?"":$tam[0]['NgayThangNamSinh']->format('d/m/Y')).'";';	
			echo 'window.GioiTinhBenhNhan="'.$tam[0]['GioiTinh'].'";';	
...
/*	function create_noigioithieu(elem,datalocal){	
	...
	function create_trangthainv(elem,datalocal){	
		...
		//datalocal=jQuery.parseJSON('{"rows":[{"id":"1","cell":["Đúng tuyến"]},{"id":"2","cell":["Có giấy chuyển viện"]},{"id":"4","cell":["Cấp cứu"]},{"id":"5","cell":["Thông tuyến"]}]} ');	 		
		 $(elem).jqGrid({
		datastr:datalocal,	
		datatype: "jsonstring" ,
		colNames:['Đối tượng khám'],
...
function them_bhyt(){
	...
	$('#chon_bhyt').click(function(){
		...
		var maThe=  $('#dm_bhyt').jqGrid('getCell', $('#dm_bhyt').jqGrid('getGridParam', 'selrow'), 'SoThe');
		var hoTen=window.HoTenBenhNhan;
		var ngaySinh=window.NgaySinhBenhNhan;//
		var gioiTinh=window.GioiTinhBenhNhan=="0"?1:0;//
		var maCSKCB= $('#dm_bhyt').jqGrid('getCell', $('#dm_bhyt').jqGrid('getGridParam', 'selrow'), 'Ma_KCB_BanDau');
		//var hsd_tu = $('#hsdtu').val();
		//alert(maThe+","+hoTen+","+ngaySinh+","+gioiTinh+","+maCSKCB+","+hsd_tu);
		if(window.barcode_bhyt==1){
			checkAndSavetheBHYT(maThe,hoTen,ngaySinh,gioiTinh,maCSKCB,hsd_tu,0,0);
		}else{
			checkAndSavetheBHYT(maThe,hoTen,ngaySinh,gioiTinh,maCSKCB,hsd_tu,1,0); 
		} 
	...
	$('#save_bhyt').click(function(e){
		...
		if((hsdtu==hsdden || noidkbd=='' || diachibh=='') && 1!=1){
			...
			var maThe= $("#mabh1").val().toUpperCase()+''+$("#mabh2").val()+''+$("#mabh3").val()+''+$("#mabh4").val()+''+$("#mabh5").val()+''+$("#mabh6").val();
				var hoTen=window.HoTenBenhNhan;
				var ngaySinh=window.NgaySinhBenhNhan;//
				var gioiTinh=window.GioiTinhBenhNhan=="0"?1:0;//
				var maCSKCB=$('#noidkbd').val();
				var hsd_tu = $('#hsdtu').val();
				//alert(maThe+","+hoTen+","+ngaySinh+","+gioiTinh+","+maCSKCB+","+hsd_tu);
				checkAndSavetheBHYT(maThe,hoTen,ngaySinh,gioiTinh,maCSKCB,hsd_tu,1,1);
		}
	});
}
		...
		loadComplete: function(data) {
			...
			if(window.barcode_bhyt==1){
				setTimeout(function(){
					...
					$("#dm_bhyt").jqGrid("setSelection",window.output_bhyt, true); 
					$("#chon_bhyt").click();
				},1000)	 
			}
...
function barcode_callback(barcode){
	...
	var maThe= res[0];
    var hoTen=$.trim(convertHexToString(res[1]));
    var ngaySinh=res[2];
    var gioiTinh=res[3];
    var maCSKCB=res[5];
    var hsd_tu = res[6];
	window.barcode_bhyt=1;
    checkAndSavetheBHYT(maThe,hoTen,ngaySinh,gioiTinh,maCSKCB,hsd_tu,0,1);
}

function checkAndSavetheBHYT(maThe,hoTen,ngaySinh,gioiTinh,maCSKCB,hsd_tu,ischeck,save=1){
	if(maThe==''){
		alert("Vui lòng nhập mã thẻ");
	}else if(hoTen==''){
		alert("Vui lòng nhập họ tên (Trong form thêm mới bệnh nhân). Rồi tắt form tạo lượt khám mở lại");
	}else if(ngaySinh==''){
		alert("Vui lòng nhập ngày sinh (Trong form thêm mới bệnh nhân). Rồi tắt form tạo lượt khám mở lại");
	}else if(hsd_tu==''){
		alert("Vui lòng nhập hạn sử dụng");
	}else{
		
	hienthongbao("Đang kết nối đến cổng BH");
    $.ajax({
		type: 'POST',
		async : false, 
				...
				data:{  
					...
					'ngaySinh': ngaySinh,
					//'gioiTinh': gioiTinh,
					//'maCSKCB': $.trim(maCSKCB.replace("-", "").split(' ').join('')),				
				...
				$("#diachibh").val(data.diaChi);
				$("#NgayDu5Nam").val(data.ngayDu5Nam);
				window.diachibh=data.diaChi; 
				$("#bhyt_info").html("");
				if(ischeck==1){
					tam ={  "000":"Thông tin thẻ chính xác",
								...
								"003":"Thẻ cũ hết giá trị sử dụng, dược cấp thẻ mới",
								"004":"Thẻ cũ còn giá trị sử dụng, dược cấp thẻ mới",
								...
								"205":"Thẻ sai định dạng tham số",
								"401":"Lỗi xác thực tài khoản"
							}
							
					$("#bhyt_info").html("");
					txt='';
					myObj=data.dsLichSuKCB2018;
					txt+='<p style="margin-bottom: 0px; margin-top: 5px;">- Kết quả: '+tam[data.maKetQua]+'</p>'; 
					txt+='<p style="margin-bottom: 0px; margin-top: 5px;">- Nơi ĐKKCB: '+data.tenDKBDMoi+'</p>';
					txt+='<p style="margin-bottom: 0px; margin-top: 5px;">- Cơ quan BHXH: '+data.cqBHXH+'</p>'; 
					txt+='<p style="margin-bottom: 0px; margin-top: 5px;">- Thông tin: '+data.ghiChu+'</p>';
					
					if (myObj != null){
						txt += "<strong>- Lịch sử KCB:</strong><br><table class='ttbh' cellspacing=0 cellpadding=0 style='width: 100%'><tr><th>Mã hồ sơ</th><th>Mã cơ sở KCB</th><th>Từ ngày</th><th>Đến ngày</th><th>Tên bệnh</th><th>Tình trạng</th><th>KQ điều trị</th><th>Lý do VV</th></tr>"
						for (var i=0;i<myObj.length;i++) {
								txt += "<tr>"
							txt += "<td style='padding:2px;'>" + myObj[i].maHoSo+ "</td>";
							txt += "<td style='padding:2px;'>" + myObj[i].maCSKCB+ "</td>";
							txt += "<td style='padding:2px;'>" + numtodate(myObj[i].ngayVao) + "</td>";
							txt += "<td style='padding:2px;'>" + numtodate(myObj[i].ngayRa)+ "</td>";
							txt += "<td style='padding:2px;'>" + myObj[i].tenBenh + "</td>";
							txt += "<td style='padding:2px;'>" + myObj[i].tinhTrang + "</td>";
							txt += "<td style='padding:2px;'>" + myObj[i].kqDieuTri + "</td>";
							txt += "<td style='padding:2px;'>" + myObj[i].lyDoVV+ "</td>";
							txt += "</tr>"
						}
						txt += "</table>" 
						...
						txt += "<p>- Không tìm thấy lịch sử KCB</p>";
					}
					myObj2=data.dsLichSuKT2018;
					if (myObj2 != null){
						txt += "<strong>- Lịch sử tra cứu gần đây:</strong><br><table class='ttbh' cellspacing=0 cellpadding=0 style='width: 100%'><tr><th>Mã cơ sở KCB</th><th>Thời gian kiểm tra</th><th>Kết quả</th><th>Thông tin</th></tr>"
						for (var i=0;i<myObj2.length;i++) {
								txt += "<tr>"
							txt += "<td style='padding:2px;'>" + myObj2[i].userKT+ "</td>";
							txt += "<td style='padding:2px;'>" + numtodate(myObj2[i].thoiGianKT)+ "</td>";
							txt += "<td style='padding:2px;'>" +tam[myObj2[i].maLoi]+ "</td>";
							txt += "<td style='padding:2px;'>" + myObj2[i].thongBao + "</td>"; 
							txt += "</tr>"
						}
						txt += "</table>";
							 
					}else{
						txt += "<p>- Không tìm thấy lịch sử tra cứu gần đây</p>";
					}
					$("#bhyt_info").html(txt)
						//data.maKetQua="000";
						...
						if(data.maKetQua!="000"){
							$("#bhyt_info").prepend('<p>Nội dung lỗi từ cổng BHYT trả về: <span style="color:red">'+tam[data.maKetQua]+'</span></p>')
							alert('Mã lỗi từ cổng BHYT trả về: '+data.maKetQua+"\nNội dung lỗi từ cổng BHYT trả về: "+tam[data.maKetQua]);
							return false;
						}
				}else{///Ischeck
					ngayDu5Nam ="";
					...
					$("#bhyt_info").html("");
						...
						if(save==0){
							...
							$.post('pages.php?module=<?=$modules?>&view=<?=$view?>&action=data_date').done(function(output){
								...
								var date_tu = new Date(dateParts2[2], (dateParts2[1] - 1), dateParts2[0]);
								window.databhyt = '{';
								window.databhyt+='"mabh":'+JSON.stringify(maThe);
								window.databhyt+=',"noidkbd":'+JSON.stringify(maCSKCB);
								window.databhyt+=',"diachibh":'+JSON.stringify($.trim(window.diachibh));
								window.databhyt+=',"hsdtu":'+JSON.stringify(hsd_tu.split('/').reverse().join('/'));
...
function chonthe(){
	...
	$.post('pages.php?module=<?=$modules?>&view=<?=$view?>&action=data_date').done(function(output){
		...
		if(oper=='add'){
			...
			var vaokham = '<?php if($_GET["oper"]=='edit') echo ($tam[0]['ThoiGianVaoKham']==null?"":$tam[0]['ThoiGianVaoKham']->format('d/m/Y'))?>';
...
function numtodate(str){ 
  return str.substring(8, 10)+':'+str.substring(10, 12)+' '+str.substring(6, 8)+'/'+str.substring(4, 6)+'/'+str.substring(0, 4);
}
</script>