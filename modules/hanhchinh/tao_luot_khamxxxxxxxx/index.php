...
<div style="margin-top:2px;">
	...
	<input id="ID_KhuVuc_hide" kiemtra="trong" type="text" style="display: none;" name="ID_KhuVuc_hide"> 
</div>  
<div style="margin-top:2px;">
...
<a style="margin-left:0px;margin-bottom:1px;" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only fm-button-icon-left" id="chonthedung_dungchobhyt" href="#">Chọn thẻ đúng<span  class="ui-icon ui-icon-check"></span></a>
...
<label for="mabhcc" style="width:100px; " >Số thẻ bảo hiểm</label>                              
<input  id="mabhcc" type="text" style="width:150px;" name="mabhcc">
</div>
<div style="margin-top:4px;">
<label for="loaithe" style="width:120px; ">Loại BHCC</label>                      
<input id="loaithe" type="text" style="width:312px;" name="loaithe"> 
</div>  
<div style="margin-top:2px;">
<label for="nhabhcc" style="width:120px; ">Nhà BHCC</label>                      
<input id="nhabhcc" type="text" style="width:312px;" name="nhabhcc"> 
</div>  
<div style="margin-top:2px;">
...
//	$tam[0]['ThoiGianVaoKham']->format('d/m/y H:i')
	echo '$("#ngay").val("'.date("d").'/'.date("m").'/'.date("Y").' '. date("H").':'.date("i").'");';
...
const urlJqGrid='resource.php?module=web_services&action=GetDataJqGrid';
const dataKhuVuc={store_name:'GD2_DM_BHYT_KhuVuc_Get',data:[],ID:'ID_KhuVuc'}

$(document).ready(function() {
	fetchData(dataKhuVuc,urlJqGrid).then(function(data){	
		create_combobox_new('#ID_KhuVuc',create_khuvuc,'bw',data,'',0);
	})
	$("#dsLichSuKCB").dialog({
		autoOpen:false,
		height:300,
		...
		$('#thuchien,#save,#chinhsua,#inphieu,#xemvain,#inphieudecal,#tamthu,#dhsinhton1,#chisinh,#hentrakq,#chon_bhyt,#lichbacsy,.button_goikham,#save_bhyt,#edit_bhyt,#new_bhyt,#save_bhcc,#edit_bhcc,#new_bhcc,#chon_bhcc,#huychon_bhcc,#btn_laysothu,#xemingl,#chonthe_bhthe2,#huychonthe2,#chonthe_bpbhyt,#chonthedung_dungchobhyt').button();
		luu();
		them_bhyt();
		them_bhcc();
		create_thongtinluotkham();
		$("#save_bhyt,#chon_bhyt,#edit_bhyt,#chonthe_bhthe2").button('disable');
		...
		create_combobox_new('#nhabhcc',create_nhabhcc,'bw','','data_nhabhcc&id_loaibhcc=0',0);    
		create_combobox_disable('#goikham');
		create_combobox_disable('#nhabhcc');
...
function create_loaithebhcc(elem,datalocal){  
	...
	$(elem).jqGrid({
		...
		onSelectRow: function(id){    
			create_combobox_new('#nhabhcc',create_nhabhcc,'bw','','data_nhabhcc&id_loaibhcc='+id,0);  
		},
		loadComplete: function(data) {  
			//grid_filter_enter(elem) ;
		},    
...
function create_nhabhcc(elem,datalocal){  
	datalocal=jQuery.parseJSON(datalocal);   
	$(elem).jqGrid({
		datastr:datalocal,
		datatype: "jsonstring",
...
function trangthai(trangthai){
	if(trangthai=='add'){  
		...
		$('#thuchien,#save,#new_bhyt,#edit_bhyt,#chon_bhyt,#chonthe_bhthe2,#new_bhcc,#edit_bhcc,#chon_bhcc,#huychon_bhcc,#btn_laysothu').button('disable');
		create_combobox_disable('#doituongbhyt');
		create_combobox_disable('#noidkbd');
		$("#mabh1,#mabh2,#mabh3,#mabh4,#mabh5,#mabh6,#mabh7,#diachibh,#hsdtu,#hsdden").prop('disabled', true);
		$("#mabhcc,#diachibhcc,#bhcc_hsdtu,#bhcc_hsdden").prop('disabled', true);
		...
		if($("#phanloai_hidden").val()==''){
			...
			if($('#phanloai_hidden').val()==24){
				...
				if(rowData['IsDungTuyen']!=1){
					if($('#trangthai_hidden').val()!=4 && $('#trangthai_hidden').val()!=2){
						var ngay =$('#ngay').val().split(' ')[0].split('/').reverse().join('/');						
						var d1 = new Date(ngay);
						var d2 = new Date('2019/1/1');
						if(d1>d2){
			...
			if(window.id_bhyt==0 && $('#doituong_hidden').val()=='BHYT'){
				...
				if(<?=$_SESSION['user']['id_user']?>==178){
					$.post("resource.php?module=<?=$modules?>&view=<?=$view?>&action=controller1&hienmaloi=2&idhenkham=<?=$idhenkham?>&id_benhnhan="+window.id_benhnhan+"&oper=add",datatosend).done(function(data) {						
		...
		if(window.id_bhyt==0 && $('#doituong_hidden').val()=='BHYT'){
			...
			if(<?=$_SESSION['user']['id_user']?>==178){
				$.ajax({
		  		...
		  		async : false,
		  		data:datatosend,
		  		url: "resource.php?module=<?=$modules?>&view=<?=$view?>&action=controller1&hienmaloi=a&id_ttluotkham="+window.id_ttluotkham+"&oper=edit&id_benhnhan=<?php 
								if($_GET["oper"]=='add'){     
				...
				$.ajax({
		  		...
		  		async : false,
				data:datatosend,		
		  		url: "resource.php?module=<?=$modules?>&view=<?=$view?>&action=controller&hienmaloi=a&id_ttluotkham="+window.id_ttluotkham+"&oper=edit&id_benhnhan=<?php 
								if($_GET["oper"]=='add'){     
									echo $_GET["id_benhnhan"];
								}else{
									echo $tam[0]['ID_BenhNhan'];
...
function them_bhyt(){
	$('#new_bhyt').click(function(){
		...
		// $('#NgayDu5Nam').button('enable');
		// $('#NgayMienCungChiTra').button('enable');
		// $('#ID_KhuVuc').button('enable');	
	...
	$('#chonthe_bpbhyt').click(function(){
		if ($('#dm_bhyt').jqGrid('getGridParam', 'selrow')==null) {
			alert("Vui lòng chọn thẻ BH");
			return false;
		}
		var check_cf=confirm("Bạn có muốn chọn thẻ này làm thẻ chính?\n (bỏ qua điều kiện ngày áp).");
		if (check_cf) {
			if(jQuery.inArray($('#dm_bhyt').jqGrid('getGridParam', 'selrow'), window.ID_ThePhu_Ar) !== -1){
				...
				$.post('resource.php?module=<?=$modules?>&view=<?=$view?>&action=data_date').done(function(output){
					...
					if(oper=='add'){
						alert("Chức năng này chỉ được sử dụng sau khi tạo lượt khám.");
					}else{
	...
	$("#chonthedung_dungchobhyt").click(function(){
		window.id_bhyt=$('#dm_bhyt').jqGrid('getGridParam', 'selrow');
		if (window.id_bhyt==null) {
			alert("Bạn chưa chọn thẻ!");
			return false;
		}
		
		var check_cf=confirm('Bạn có muốn áp thẻ đang chọn làm thẻ chính của lượt khám này?');
		if (check_cf) {
			$.ajax({
				type: 'POST',async : false,dataType: 'json',
				url: 'resource.php?module=<?= $modules ?>&view=<?= $view ?>&action=controller_chonthe_bpbhyt&oper=chonthe_bp_bhyt',
				data: {
					id_luotkham:window.id_ttluotkham,
					id_the:window.id_bhyt
				},
				success: function(response) {
					tooltip_message("Chọn thẻ thành công!");
		...
		if(oper=='add'){
			...
			if(window.id_ttluotkham>0){
				if(window.LoaiDoiTuongKham=='BHYT'){
					...
					$.post("resource.php?module=<?=$modules?>&view=<?=$view?>&action=controller_apthephu&hienmaloi=a&id_ttluotkham="+window.id_ttluotkham+"&idthe_phu="+removeItem+"&ac=bhyt_huychon_thephu").done(function(data) {
						data=$.parseJSON(data);
							if(data.Error!=1){
				...
				if(date_den<date_vaokham){
					...
					if(window.id_ttluotkham>0){
						if(window.LoaiDoiTuongKham=='BHYT'){
							...
							$.post("resource.php?module=<?=$modules?>&view=<?=$view?>&action=controller_apthephu&hienmaloi=a&id_ttluotkham="+window.id_ttluotkham+"&idthe_phu="+window.ID_BHYT_ThePhu+"&ac=bhyt_thephu").done(function(data) {
								data=$.parseJSON(data);
								if(data.IsLock==1){
									alert(data.Notes);
								}else{  
...
$('#save_bhyt').click(function(e){
	...
	if(hsdtu==hsdden || noidkbd=='' || diachibh==''){
		...
		window.databhyt+=',"ID_KhuVuc":'+JSON.stringify($('#ID_KhuVuc_hide').val())+'';
	...
	function them_bhcc(){
		...
		$('#new_bhcc').click(function(){   
			...
			create_combobox_enable('#nhabhcc');
		...
		$('#edit_bhcc').click(function(){
			...
			if(rowData['ID_LoaiTheBHCC']==45 || rowData['ID_LoaiTheBHCC']==46){
				alert('thẻ ivf không được sửa');
				return false;
			}			
			...
			setval_new('#nhabhcc',rowData['ID_NhaBHCC']); 
			$('#nhabhcc').val(rowData['TenNhaBHCC']);
			$('#nhabhcc_hidden').val(rowData['ID_NhaBHCC']);
			window.bhcc_ac='edit';
		...
		$('#chon_bhcc').click(function(){  
			if(oper=='add'){
				if($('#chuyenkhoa_hidden').val()==58){
					alert('Lượt khám vô sinh không được chọn thẻ');
					return false;
				}				
				window.id_bhcc=$('#dm_bhcc').jqGrid('getGridParam', 'selrow');
		...
		$('#huychon_bhcc').click(function(){
			if(oper=='add'){
				...
				if($('#chuyenkhoa_hidden').val()==58){
					alert('Lượt khám vô sinh không được hủy thẻ');
					return false;
				}
				if(window.id_ttluotkham>0){
					window.id_bhcc=$('#dm_bhcc').jqGrid('getGridParam', 'selrow');
		...
		$('#save_bhcc').click(function(e){
			...
			window.databhcc+=',"nhabhcc":'+JSON.stringify($('#nhabhcc_hidden').val())+'';
			window.databhcc+='}';
		...
		function create_dm_bhcc(){
			$('#dm_bhcc').jqGrid({
				...
				datatype: "json",
				colNames:['Số thẻ','Địa chỉ', '', 'Loại thẻ', 'HSD từ','HSD đến','NgayCap','ID_NhaBHCC','Nhà BHCC'],
				colModel:[
					{name:'SoThe',index:'SoThe',search:true,hidden :false,width:'10%'},
					{name:'DiaChiTheBHCC',index:'DiaChiTheBHYTCC',search:true,hidden :true,width:'15%'},
					...
					{name:'NgayCap',index:'NgayCap',hidden :true},
					{name:'ID_NhaBHCC',index:'ID_NhaBHCC',hidden :true},
					{name:'TenNhaBHCC',index:'TenNhaBHCC',hidden :false,width:'8%'},
				],
				hidegrid: false,
				gridview: true,
				loadonce: true,
				...
				return false;
		  		...
		  		async : false,
				  		...
				  		success: function(data, status, xhr) {
				  			...
				  			if ($.trim(data)==''){
				  				alert('Không thể kết nối tới máy chủ BHYT')
				  			}
