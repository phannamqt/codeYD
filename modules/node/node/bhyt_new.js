...
io.on('connection', function(socket) {
	socket.on('all', function(data) {		
		data=data.split('|');
		xuat_xml(data[0],data[1],1,"F:/bhyt_4210/");
		console.log("all");
	})
	socket.on('list', function(data) {	
		...
		lanchay=0;
		xuat_xml(thongtinluotkham[0]['ID_ThuTraNo'],thongtinluotkham[0]['noitru'],2,"F:/bhyt_4210");	
		console.log("list");
	})
})
...
function xuat_xml(id_thutrano,noitru,tinhtrang,duongdan){	
		...
		sql.connect(config, function(err) {
			...
			request.execute(store_name, function(err, recordsets, returnValue) {   	
				if(err === undefined){
					...
					THANG_QT=formatTimezoneOffset(thongtin.NGAY_RA).getMonth()+1;
					NGAYLAP=formatDate(thongtin.NGAY_RA);
					...
					for(var i=0;i<thongtincls.length;i++){	 
						...
						if(thongtincls[i]['ID_BHYT']==10){
							...
							chitietcls+='<MA_BAC_SI>'+thongtin['MA_BAC_SI']+'</MA_BAC_SI>';	
					...
					if(thongtinthuoc.length>0){							
						...
						for(var i=0;i<thongtinthuoc.length;i++){
							...
							chitietthuoc+='<TT_THAU>1</TT_THAU>';
							...
							chitietthuoc+='<MA_BAC_SI>'+thongtinthuoc[i]['MA_BAC_SI']+'</MA_BAC_SI>';
					...
					tonghop+='<T_VTYT>0</T_VTYT>';
					tonghop+='<T_TONGCHI>'+T_TONGCHI+'</T_TONGCHI>';
					tonghop+='<T_BNTT>0</T_BNTT>';
	...
	lanchay=lanchay+1;
	if(lanchay<soca){
		xuat_xml(thongtinluotkham[lanchay]['ID_ThuTraNo'],thongtinluotkham[lanchay]['noitru'],0,"F:/bhyt_xml_new/");
	}
}
}
	...
	sql.connect(config, function(err) {
		...
		request5.execute('GD2_BHYT_xml_chuachuyen', function(err, recordsets, returnValue) {   
			...
			if(	thongtinluotkham.length>0){				
				xuat_xml(thongtinluotkham[0]['ID_ThuTraNo'],thongtinluotkham[0]['noitru'],0,"F:/bhyt_xml_new/");				
			}						
			setTimeout(function(){				
				caccachuathanhtoan();			
			},600000)
