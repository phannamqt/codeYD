...
var config = {
	user: 'qlbv',
	password: 'qlbv@123',
    server: '192.168.1.5', // You can use 'localhost\\instance' to connect to named instance 
    database: 'MedSmart',    
    options: {
...
io.on('connection', function(socket) {
	socket.on('all', function(data) {		
		data=data.split('|');
		xuat_xml(data[0],data[1],1,"F:/bhyt_xml/BHYT/");
		console.log("all");
	})
	socket.on('list', function(data) {	
		...
		lanchay=0;
		xuat_xml(thongtinluotkham[0]['ID_ThuTraNo'],thongtinluotkham[0]['noitru'],2,"F:/bhyt_xml/BHYT/");	
		console.log("list");
	})
})
						...
						if(tinhtrang==0){
							...
							request3.execute('GD2_BHYT_xml_DaChuyen_Update', function(err, recordsets, returnValue) {   
								...
								if(lanchay<soca){
									...
									xuat_xml(thongtinluotkham[lanchay]['ID_ThuTraNo'],thongtinluotkham[lanchay]['noitru'],0,"F:/bhyt_xml/bhyt_xml/");
								}
							})
						}else if(tinhtrang==1){
							sql.close();
	...
	sql.connect(config, function(err) {
		...
		request5.execute('GD2_BHYT_xml_chuachuyen', function(err, recordsets, returnValue) {   
			...
			if(	thongtinluotkham.length>0){				
				xuat_xml(thongtinluotkham[0]['ID_ThuTraNo'],thongtinluotkham[0]['noitru'],0,"F:/bhyt_xml/bhyt_xml/");				
			}						
			setTimeout(function(){				
				caccachuathanhtoan();			
			},600000)
