...
<?php
	...
	$store_name="{call [GD2_KSK_TrangThai] (?)}";//tao bien khai bao store
          ...
          <?php 
					...
					foreach ($thongtin as $key => $value) {
						...
						foreach($value as $key2 => $value2){
				 			...
				 			if($t2>12 && ($t2%2==0))// lấy từ cột thứ 2 và lấy trên chỉ số chẵn của key là ra column
			 ...
			 for( $i=0;$i<count($thongtin);$i++){
				...
				for($j=0;$j<$cols/2;$j++)
				{
					...
					echo'<td >'. $thongtin[$i][$j].'</td>';		
						if($thongtin[$i][$j]!=NULL and $j>5)//chỉ cộng từ cột tiền dịch vụ, tránh lỗi cộng các cột ko phải là tiền dịch vụ
						{
						$sum_row+=$thongtin[$i][$j];
						if($thongtin[$i][3]!=NULL)
						{
							$sum_conlai=$thongtin[$i][3]-$sum_row;
						}
