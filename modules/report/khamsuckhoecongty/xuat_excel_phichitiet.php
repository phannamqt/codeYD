...
<?php
	...
	$store_name="{call [GD2_KSK_SumPhiThucHien_New] (?)}";//tao bien khai bao store
          ...
          <?php 
					...
					foreach ($thongtin as $key => $value) {
						...
						foreach($value as $key2 => $value2){
				 			$t2++;
				 			if($t2>14 && ($t2%2==0))// lấy từ cột thứ 15 và lấy trên chỉ số chẵn của key là ra column
							{   
								//echo substr($key2,1).' - '.$t2.' ; ';
								$t3++;
             ...
             <th >Tổng tiền thuốc và các điều trị khác</th>
          ...
          <tbody id="tbody_2">
			 ...
			 for( $i=0;$i<count($thongtin);$i++){
				...
				for($j=0;$j<$cols/2;$j++)
				{
					...
					//echo'<td >$i='.$i.'$j='.$j.'-'. $thongtin[$i][$j].'</td>';
					if($j==2){
						$thongtin[$i][$j]="'".$thongtin[$i][$j]; 
					}
					echo'<td >'. $thongtin[$i][$j].'</td>';		
						if($thongtin[$i][$j]!=NULL and $j>6)//chỉ cộng từ cột tiền dịch vụ, tránh lỗi cộng các cột ko phải là tiền dịch vụ
						{
							$sum_row+=$thongtin[$i][$j];
							if($thongtin[$i][4]!=NULL)
							{
								$sum_conlai=$thongtin[$i][4]-$sum_row;
							}
				...
				echo'<td align="right"><font color="blue">'. $sum_row.'</font></td>';
				
				echo'<td align="left"> <font color="red">'. $sum_conlai  .'</font></td>';
				$sum_datra=0;
				$sum_row=0;
				 $sum_conlai=0;
				echo '</tr>';	   
