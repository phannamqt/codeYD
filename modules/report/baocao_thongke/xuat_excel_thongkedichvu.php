...
if (date('H') >=8 && date('H') <=10 && $_SESSION["user"]["id_user"]!=74 && $_SESSION["user"]["id_user"]!=8 
    && $_SESSION["user"]["id_user"]!=67
    && $_SESSION["user"]["id_user"]!=7
    && $_SESSION["user"]["id_user"]!=178
    && $_SESSION["user"]["id_user"]!=785
     ) 
{
//if (date('H') <16 && $_SESSION["user"]["id_user"]!=53 && $_SESSION["user"]["id_user"]!=8 && $_SESSION["user"]["id_user"]!=178&& $_SESSION["user"]["id_user"]!=74) {
 echo "<h1><center>Chức năng này đang tạm khóa từ 8h đến 11h vì ảnh hưởng tốc độ hệ thống!</center></h1>";
 exit();
}

$data = new SQLServer; //tao lop ket noi SQL

    $params = array($_GET["id_nhanvien"],$_GET["id_loaikham"],$_GET["id_nhomcls"],convert_date($_GET['fromdate']),convert_date($_GET['todate']).' 23:59:59');//tao param cho store
    //print_r($params);
...
<div id="wrapper">
    ...
    /*        $responce->rows[$i]['cell']=array($i,$row["TaoLuc"],$row["TenLoaiKham"],$row["TenNhom"],$row["HoLotBenhNhan"],
    $row["TenBenhNhan"],
    $row["MaBenhNhan"], $row["namsinh"],
    $row["ChiDinh"], $row["ThucHien"],
    $row["HoanTat"],*/
    ?>
