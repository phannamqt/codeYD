...
$store_name = "{call [GD2_ThuChi_No_KeToan_Group] (?,?)}"; //tao bien khai bao store
...
<body>
<div id="wrapper">
	<div style="text-align:center;font-size:18px; font-weight:bold">BÁO CÁO CÔNG NỢ TẤT CẢ BỆNH NHÂN KẾ TOÁN GỘP.(ĐƠN VỊ TÍNH 1000đ) </div>
    <?php
	//if($_GET['theonhacungcap']=='false'){
		/*echo '<div style="text-align:center;font-size:15px; font-weight:bold">'.$_GET["ten_kho"].' </div>';*/
		echo '<div style="text-align:center;font-size:15px; font-weight:bold"> Đầu kỳ từ '.$_GET["fromdate"].' cuối kỳ đến '.$_GET["todate"].'</div>';
	//}
	//else{
    ...
    <table border="1" cellpadding="0" cellspacing="0" align="center" style="width:1500px;margin-top:5px">
    	<tr height="30">
    		...
    		<th>Nợ cũ(đầu kỳ)</th>
    		<th>Phát sinh trong kỳ(phải trả sau cùng)</th>
    		<th>Đã trả</th>
             <th>BHYT nợ</th>
            <th>BHCC nợ</th>
    		<th>Nợ mới BN</th>
		...
		?>
         <tr>
             ...
             <?php if($row["PhatSinh"]==0)
             {
              
                echo '<td bgcolor="pink">'.$row['MaBenhNhan'].'</td>';
                  echo '<td bgcolor="pink">'.$row['HoLotBenhNhan'].'</td>';
                    echo '<td bgcolor="pink">'.$row['TenBenhNhan'].'</td>';
               
             }
             else
             {
                 echo '<td  bgcolor="FFF">'.$row['MaBenhNhan'].'</td>';
                  echo '<td  bgcolor="FFF">'.$row['HoLotBenhNhan'].'</td>';
                   echo '<td  bgcolor="FFF">'.$row['HoLotBenhNhan'].'</td>';
         	...
         	<td align="right" ><?= $row['NoDauKy']?></td>
         	<td align="right" ><?= $row['TongPhaiTraSauCung']?></td>
...
<?php
	if($types=="excel"){
		...
		$exp->exportWithPage("excel/temp.xls","baocaoThuChi_CongNo_KeToan_Gop.xls");
	}
	?>