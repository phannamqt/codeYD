...
<?php
	...
	$store_name="{call TongHopThuocHangNgay_new(?,?)}";//tao bien khai bao store
...
<body>
        ...
        <table id="bang_1" width="1200px" cellpadding="0" cellspacing="0" border="1">
          <tr>
            ...
            <th width="10%">Khoa Nhi</th>
            <th width="10%">Khoa Sản</th>
            <th width="10%">Khoa HSTC-CĐ</th>
            <th width="10%">Tổng</th>
          </tr>
          <tbody id="tbody_1">
              <?php 
				...
				$count=1;
			  	for( $i=0;$i<count($result);$i++){
					...
					if($result[$i]['KhoaNhi']==NULL){$result[$i]['KhoaNhi']=0;}
					if($result[$i]['KhoaSan']==NULL){$result[$i]['KhoaSan']=0;}
					if($result[$i]['KhoaHSTC']==NULL){$result[$i]['KhoaHSTC']=0;}
					...
					if($i==0||$result[$i]['NgayXuat']!=$temp)
					{
						...
						echo '<td>'.trim($result[$i]['ID_Thuoc']).'</td>';
						echo '<td>'.trim($result[$i]['TenGoc']).'</td>';
						echo '<td>'.trim($result[$i]['TenDonViTinh']).'</td>';
						...
						echo '<td align="right">'.$result[$i]['KhoaNhi'].'</td>';
						echo '<td align="right">'.$result[$i]['KhoaSan'].'</td>';
						echo '<td align="right">'.$result[$i]['KhoaHSTC'].'</td>';
						$sum=(int)$result[$i]['KhoaGayMeHoiSuc']+(int)$result[$i]['KhoaHoiSucCapCuu']+(int)$result[$i]['KhoaNgoai']+(int)$result[$i]['KhoaNoi']+(int)$result[$i]['KhoaNhi']+(int)$result[$i]['KhoaSan']+(int)$result[$i]['KhoaHSTC'];
