  ...
  $.fn.printPage = function(options) {
    ...
    function loadPrintDocument(el, pluginOptions){
      ...
      $("#printMessageBox").animate({opacity:1}, 300, function() { 
	 	 addIframeToPage(el, pluginOptions, printit); 
	  });
    }
    /**
     ...
     * @param {Object} pluginOptions - options for this print button
     */
    function addIframeToPage(el, pluginOptions,callback){
        var url = (pluginOptions.url) ? pluginOptions.url : $(el).attr(pluginOptions.attr);
		var data = pluginOptions.data
        if(!$('#printPage')[0]){		 
          $('#printPage').bind("load",function() { callback();  })
        }else{		 
    ...
    var components = {
      iframe: function(url){
        return '<iframe id="printPage" name="printPage" src='+url+' style="position:absolute;top:0px; left:0px;width:1000px; height:400px;border:0px;overfow:none; z-index:1000"></iframe>';
      },
