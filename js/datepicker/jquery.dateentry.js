...
$.extend(DateEntry.prototype, {
	...
	_setDate: function(inst, date) {
		...
		date = this._normaliseDate(
			...
			//console.log(this._determineDate(date || inst.options.defaultDate, inst) );
			//console.log(this._determineDate(date || inst.options.defaultDate, inst));
	...
	_handleKeyPress: function(inst, chr) {
			//sua chua
		if (typeof $(inst.input).data('dateentry') == 'undefined'){
					$(inst.input).data('dateentry',0);
				}
		//alert(inst.options.dateFormat.substring(3));
		if (inst.options.dateFormat.substring(3).indexOf(chr) > -1) {
			...
			var day = (field != 'd' && field != 'w' && field != 'W' ?
				...
				(key > 0 ? key : inst._selectedDay)));
					...
					//sua chua
				if(field == 'd' || field == 'w' || field == 'W'){					
					if(value > this._getDaysInMonth(year, month - 1)){
						//console.log(value);
						if(value==31){
							var day=31;
							var month=1;
			...
			inst._lastChr = (field != 'y' ? '' :
				...
				}else if(field == 'd' || field == 'w' || field == 'W'){
					if (chr >= '4' && chr <= '9'){
							this._changeField(inst, +1, false);
					}else{
						if($(inst.input).data('dateentry')==0){
							$(inst.input).data('dateentry',1)
						}else if($(inst.input).data('dateentry')==1){
							this._changeField(inst, +1, false);
						}
				}
				}else if(field == 'm' || field == 'n' || field == 'N'){
					if (chr >= '2' && chr <= '9'){
							this._changeField(inst, +1, false);
					}else{
						if($(inst.input).data('dateentry')==0){
