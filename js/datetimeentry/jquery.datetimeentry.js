...
function DatetimeEntry() {
	...
	this.regional[''] = { // Default regional settings
		datetimeFormat: 'H:M D/O/y', // The format of the date text:
	...
	_handleKeyPress: function(inst, chr) {
		...
		if ((inst.options.datetimeSeparators + sep).indexOf(chr) > -1) {
			...
			inst._lastChr = (field != 'Y' ? '' :
				...
				//sua chua
				//truong hop gio` trc ngay sau
				if(inst.options.datetimeFormat=='H:M D/O/y'){
					if(field=='H'){
						if (chr >= '3' && chr <= '9'){
							this._changeField(inst, +1, false);
						}else{
				...
				}else if(inst.options.datetimeFormat=='D/O/y H:M'){
					if(field=='H'){
						if (chr >= '3' && chr <= '9'){
							this._changeField(inst, +1, false);
						}else{
