...
(function ($) {
    ...
    function SlickGrid(container, data, columns, options) {
        ...
        function init() {
            ...
            $canvasTopL = $("<div class='grid-canvas grid-canvas-top grid-canvas-left phannam' tabIndex='0' hideFocus />").appendTo($viewportTopL);
            $canvasTopR = $("<div class='grid-canvas grid-canvas-top grid-canvas-right phannam' tabIndex='0' hideFocus />").appendTo($viewportTopR);
        ...
        function finishInitialization() {
            if (!initialized) {
                ...
                $canvas
                    ...
                    .delegate(".slick-cell emr-cell", "mouseenter", handleMouseEnter)
                    .delegate(".slick-cell emr-cell", "mouseleave", handleMouseLeave);
            }
        }
        ...
        function measureCellPaddingAndBorder() {
            ...
            el = $("<div class='slick-cell emr-cell' id='' style='visibility:hidden'>-</div>").appendTo(r);
		...
		function getDataItemValueForColumnNAM(item, columnDef) {
            return item[columnDef.field];
        }
        ...
        function appendRowHtml(stringArrayL, stringArrayR, row, range, dataLength) {
			...
			if(d){
				...
				var n_class="nam-show-row";
				if(d.__group==true){
					if(d.rows[0].id_group)
						id_group=d.rows[0].id_group;
				}
				//if(d.rows[0].id_group)
				//console.log(d.rows[0].id_group);
			}else{
				var n_rowid=0;
				var n_class="nam-hide-row";
			}
			
            var rowCss = "slick-row" +
                (dataLoading ? " loading" : "") +
                (row === activeRow ? " active" : "") +
                (row % 2 == 1 ? " odd phannam-chan" : " even phannam-le");
            ...
            var rowHtml = "<div id-group='"+id_group+"' nam-id='"+n_rowid+"' class='ui-widget-content " + rowCss+" "+n_class + "' style='top:"
        ...
        function appendCellHtml(stringArray, row, cell, colspan, item) {
            ...
            var m = columns[cell];
            var cellCss = "emr-cell slick-cell l" + cell + " r" + Math.min(columns.length - 1, cell + colspan - 1)
			...
			if (item) {
				...
				if(getDataItemValueForColumn(item, m)){
					var nam_values = getDataItemValueForColumn(item, m);
				}else{
					var nam_values ='';
				}
			}else{
            ...
            stringArray.push("<div class='emr-custom-class " + cellCss + "' title='"+nam_values+"'>");
        ...
        function getRenderedRange(viewportTop, viewportLeft) {
			...
			//console.log(range.leftPx);
			// CÓ THỂ GÂY NẶNG NHƯNG CHƯA CHẮC DOẠN IF Ở DƯỚI
			if(range.leftPx>1){
				range.leftPx = Math.max(0, 1);// nam edit de load data khi scroll qua phai khong hien thi forzen truoc
			}else{
				range.leftPx = Math.max(0, range.leftPx);// Cach tinh cu
			}
